...
The framework rests on a simple observation. The $2\times C$ \textbf{empirical output channel matrix} $\mathbf{M}(\theta')$ has rows $\boldsymbol{\mu}_f$ and $\boldsymbol{\mu}_r$ (mean softmax vectors on $D_f$ and $D_r$). Its singular values $\sigma_1 \ge \sigma_2$ encode residual dependence: $\sigma_2 = 0 \iff \mathrm{rank}(\mathbf{M}) = 1 \iff$ perfect removal.
