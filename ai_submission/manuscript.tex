...
\item A proof (Theorem~\ref{thm:perfect}) that $\rho=0$ iff $I(X;Y\mid\theta')=0$ via a channel-level argument that requires no exponential-family or calibration assumptions.
...
The bound is local: it holds while the trajectory stays in the $L$-Lipschitz regime of $\theta_0$, which the small-step condition enforces. Equivalently, one may normalize the ascent direction and absorb $1/\|\mathbf{g}\|_2$ into the definition of $\lambda$; the statement is unchanged up to a rescaling of the effective step. At the learning rates used in practice ($\lambda{\sim}10^{-5}$ with $\|\mathbf{g}\|_2$ of order one), the condition is comfortably satisfied.
\end{remark}
...
\paragraph{Cross-dataset and sensitivity.} $\rho\le2\times10^{-3}$ for $C{=}10$--$100$ (MNIST, CIFAR-10/100), confirming the rank-one property is structural. Under deliberate overfitting (Table~\ref{tab:sensitivity}, Sec.~\ref{sec:sensitivity}), $\rho$ surges $10\times$ (MNIST 60K$\to$5K, 50 epochs: $9.8\times10^{-4}\to1.24\times10^{-2}$). The controlled $\alpha$-perturbation confirms orthogonal axes: $\alpha{=}0.20$ raises RII $9\times$ while MIA rises by $6.8$ percentage points. ResNet-18 on CIFAR-100 keeps $\rho{=}1.41\times10^{-3}$ (Table~\ref{tab:resnet18}), confirming architecture independence.
