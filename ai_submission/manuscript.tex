...
\textbf{Notation for MIA.} The abbreviation ``MIA'' denotes three distinct protocols across the paper, and each table states its own: train/test membership-inference accuracy in the random-forgetting baseline (Table~\ref{tab:main}); a forget/retain loss-threshold AUC in the class-level benchmark (Table~\ref{tab:benchmark}); and the forget-set mean next-token NLL on TOFU (Table~\ref{tab:llm}). Where the two leakage axes are contrasted (Sec.~\ref{sec:mia}), ``MIA'' refers to the train/test membership accuracy.

Three patterns recur below. Standard training induces near-rank-one output channels ($\rho<5\times10^{-3}$), making RII a stable, tuning-free quantity: random sample-level forgetting is trivially indistinguishable, whereas class-level forgetting is not (Sec.~\ref{sec:benchmark}). Under class-level forgetting, RII and MHPR rank unlearning methods consistently with forgetting strength and expose methods that leave---or even strengthen---an output signature that accuracy and MIA miss; on a 7B-parameter language model with the TOFU benchmark (Section~\ref{sec:llm}) the same ordering holds even when all unlearned checkpoints saturate generation accuracy at 100\%. Finally, RII and MIA measure overlapping but distinct leakage axes ($r{=}0.81$), so a complete audit should report both.
