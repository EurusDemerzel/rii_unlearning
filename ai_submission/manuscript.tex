...
Class-level unlearning verification sits between two failure modes: accuracy-based metrics saturate and lose their ordering power at high forgetting strength, while MIA measures a related but distinct quantity---general training membership---rather than forget-set--specific output leakage. RII addresses this gap: it is a tuning-free, black-box certificate of output-level indistinguishability that remains informative precisely when accuracy saturates. Empirically, it ranks seven unlearning methods consistently with forgetting strength across CIFAR-10, Fashion-MNIST, and CIFAR-100, and on TOFU with LLaMA-2-7B it retains that ranking among the checkpoints whose generation accuracy is saturated at 100\%. The experiments also delimit what a spectral certificate can and cannot do. RII \emph{rises} under gradient ascent even as accuracy and MIA improve (a failure it exposes, and one that strengthens with class count), it rebounds after ascent-then-fine-tune, and it falls toward zero when a model degenerates, so it must always be read together with utility metrics. On the theory side, $\rho=0$ is equivalent to output-channel independence, the fine-tuning bound is $O(\lambda^2)$, and the unified decomposition splits leakage into a direct (MIA) and a cover (RII) axis. The MHPR extension and its temperature scaling handle the known/unknown class asymmetry, and PSSD (Appendix~\ref{app:pssd}) extends the audit to per-sample state disparity.
