...
Unlike the spectral RII $\rho\in[0,0.5]$ (a normalized second singular value of the $2\times C$ channel), $\rho_H$ is the \emph{relative energy of the projection residual} onto the held-out subspace; the different normalizations (spectral energy ratio vs.\ geometric residual norm ratio) account for the different ranges.
...
\textbf{Limitations.} The certificate is output-level by construction: it does not bound leakage in internal representations, which the probe experiments show can persist even for the retraining oracle, and it does not certify parameter-level privacy. The class-level studies are single-run (training cost); seed stability is established on the random baseline (10 seeds) and the 7B study (3 subsamples). The AG News and 0.5B results are single-run pilot observations reported as transfer evidence rather than full benchmarks. The MI bound requires $\pi_{\min}>0$, and the unified decomposition is exact under the Gaussian signal model, holding to leading order more generally.

\textbf{Future work.} Directions include a formal DP--RII connection, extending the author-level TOFU study to the full benchmark suite (forget 90/95/99, Truth Ratio and KLR~\cite{maini2024tofu}) and to MUSE~\cite{shi2024muse}, class-level output channels (e.g.\ author-classification heads) for language models, the multi-layer state-trajectory extension of PSSD (Appendix~\ref{app:pssd}), combining RII with internal representation audits for full white-box certification, and ImageNet-scale validation.
