...
\label{rem:pi_min}
For the empirical channel, $(\boldsymbol{\mu}_r)_y$ is the relative frequency of class $y$ on the retain set, so $\pi_{\min}>0$ holds whenever every class appears at least once---the case in all experiments here. For population channels with a very large number of classes, where some classes may carry arbitrarily small probability, the bound can be applied to the truncated support $\mathcal{Y}_\epsilon=\{y:(\boldsymbol{\mu}_r)_y>\epsilon\}$, paying a multiplicative $O(1/\epsilon)$ factor; the same truncation appears in the confidence bounds (Proposition~\ref{prop:confidence}).
\end{remark}
