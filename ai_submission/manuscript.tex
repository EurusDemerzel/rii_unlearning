...
\paragraph{Cross-dataset and sensitivity.} $\rho\le2\times10^{-3}$ for $C{=}10$--$100$ (MNIST, CIFAR-10/100), confirming the rank-one property is structural. Under deliberate overfitting (Table~\ref{tab:sensitivity}, Sec.~\ref{app:extra}), $\rho$ surges $10\times$ (MNIST 60K$\to$5K, 50 epochs: $9.8\times10^{-4}\to1.24\times10^{-2}$). The controlled $\alpha$-perturbation confirms orthogonal axes: $\alpha{=}0.20$ raises RII $9\times$ while MIA rises by $6.8$ percentage points. ResNet-18 on CIFAR-100 keeps $\rho{=}1.41\times10^{-3}$ (Table~\ref{tab:resnet18}), confirming architecture independence.
