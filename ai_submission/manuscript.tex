...
Table~\ref{tab:dynamic_range} shows $\rho$ for class forgetting is \textbf{152$\times$ larger} than random forgetting, demonstrating RII is not degenerate. RII decreases with gradient ascent steps after an initial transient increase at the first step (Table~\ref{tab:gradient_sweep}), consistent with the $O(\lambda^2)$ bound of Eq.~\eqref{eq:finetune_bound} (Theorem~\ref{thm:finetune}), where $\lambda$ is the ascent step size.
