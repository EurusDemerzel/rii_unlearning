...
\item \textbf{We design} MHPR (Sec.~\ref{sec:mhpr}) for class-level forgetting: a multi-held-out projection that largely resolves the known/unknown class asymmetry (reducing the oracle's residual well below its standard-RII level), with temperature scaling and finite-sample guarantees.
...
\item \textbf{We evaluate} RII and MHPR on a unified seven-method benchmark and on the TOFU benchmark with a 7B-parameter language model (Sec.~\ref{sec:experiments}), exposing hidden failures (gradient-ascent signature strengthening and fine-tune rebound) that accuracy and MIA miss; on TOFU, where all unlearned methods saturate at 100\% generation accuracy, RII is the only metric that separates retraining from no unlearning. We also give an $O(CN)$ audit protocol and a per-sample extension, Per-Sample State Disparity (PSSD; Appendix~\ref{app:pssd}).
\end{enumerate}
...
\label{thm:finetune}
For one-step gradient ascent $\theta' = \theta_0 + \lambda\nabla_\theta\mathcal{L}(\theta_0, D_f)$ with $L$-Lipschitz gradient and $L$-Lipschitz softmax map,
\begin{equation}
...
\centering
\small
\caption{Rank-one baseline: RII under standard training (5\% random forget; all rows use NoUnlearn).}\label{tab:main}
...
Our experiments reveal an apparent paradox: MIA accuracy remains around 90\% in the random-forgetting baseline (Table~\ref{tab:main}) while $\rho \approx 10^{-3}$. Theorem~\ref{thm:unified} resolves this: MIA and RII measure orthogonal dimensions.
