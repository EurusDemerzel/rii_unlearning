...
\textbf{Future work.} Directions include a formal DP--RII connection, extending the author-level TOFU study to the full benchmark suite (forget 90/95/99, Truth Ratio and KLR~\cite{maini2024tofu}) and to MUSE~\cite{shi2024muse}, class-level output channels (e.g.\ author-classification heads) for language models, the multi-layer state-trajectory extension of PSSD (Appendix~\ref{app:pssd}), combining RII with internal representation audits for full white-box certification, ImageNet-scale validation, evaluating recent state-of-the-art unlearning methods such as SCRUB~\cite{kurmanji2023scrub} and SalUn~\cite{fan2024salun} alongside the gradient-based baselines, and---when semantically matched held-out classes are unavailable---constructing MHPR reference subspaces from synthetic or generative reference distributions.
