...
\begin{abstract}
Machine unlearning aims to remove the influence of specified training data from a trained model. Verifying whether such removal has been effective remains an open and practically important problem. Membership-inference attacks (MIA) conflate train/test shift with forget-set leakage, and differential-privacy bounds require invasive training, so neither certifies output-level indistinguishability. We introduce a spectral certificate built on a $2\times C$ \emph{empirical output channel matrix} whose rows are the averaged softmax predictions on the forget and retain sets, and define the \textbf{Residual Irreversibility Index (RII)} $\rho$, which vanishes exactly when the two channel output distributions are indistinguishable. For class-level forgetting, where a known/unknown class asymmetry inflates RII, we extend it to the \textbf{Multi-Held-Out Projection Residual (MHPR)}, which projects the forgotten class mean onto multiple held-out unseen classes and comes with finite-sample guarantees and temperature scaling for low-accuracy regimes. We prove that $\rho=0$ is equivalent to output-channel independence and derive fine-tuning-leakage, mutual-information, and unified two-axis-decomposition bounds. On a unified benchmark across CIFAR-10, Fashion-MNIST, and CIFAR-100, and on the standard TOFU benchmark~\cite{maini2024tofu} and a MUSE-style long-form book-unlearning protocol with a 7B-parameter language model (LLaMA-2-7B, LoRA-based PEFT), RII ranks unlearning methods consistently with known forgetting strength in most settings and remains informative where accuracy and MIA saturate. When the unlearned checkpoints saturate forget-set generation accuracy at 100\%, accuracy alone cannot rank them, whereas RII still orders fine-tuning below no-unlearning. RII also exposes a hidden failure of gradient ascent: its output signature strengthens even as accuracy and MIA report improvement, together with a fine-tune rebound that accuracy-based metrics miss. On long-form book unlearning (MUSE style), gradient ascent leaves the forget output distribution unchanged, forget perplexity ranks retraining as the strongest erasure, and raw-vocabulary channels carry the two books' inherent distinctness rather than the forgetting state, delimiting the regimes in which RII is informative. Representation-level probes confirm that output-level erasure can coexist with feature-level signatures, motivating a two-layer audit.
