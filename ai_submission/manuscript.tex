...
\bottomrule
\end{tabular}
\par\vspace{2pt}\footnotesize MIA-loss is a loss-threshold AUC; the value $0.000$ for Retrain is expected (the oracle never trained on the forget class, giving a perfect reverse ranking; see Sec.~\ref{sec:benchmark}).
\end{table}

Even the retraining oracle (mathematically perfect unlearning) has nonzero RII, $\rho{=}9.15{\times}10^{-2}$: because cat is now an \emph{unseen} class, its output distribution differs from the retain classes, so standard RII cannot certify the oracle as perfect under class-level forgetting. This known/unknown asymmetry is what MHPR addresses, narrowing the gap to NoUnlearn ($0.428$ vs $0.812$).
...
\centering
\small
\caption{Pearson correlations of evaluation metrics across the seven unlearning methods (CIFAR-10; correlations are over the seven method-level points, so treat as indicative).}
\label{tab:corr}
...
RII and MHPR are strongly consistent ($r{=}0.91$) and track forget accuracy ($r{=}0.93$ and $0.87$); the $r{=}0.93$ RII--accuracy correlation is driven by the Retrain endpoint (forget accuracy $0\%$), and among the non-oracle methods the two metrics diverge systematically: NegGrad's forget accuracy \emph{falls} (from $67.9\%$ to $63.4\%$) while its RII \emph{rises} (from $0.233$ to $0.254$), a failure an accuracy proxy would miss. RII correlates moderately-to-strongly with MIA-loss ($r{=}0.81$) and disagrees at the top (NegGrad is worst under RII but second-worst under MIA-loss), supporting the orthogonal-axes view (Section~\ref{sec:mia}). Representation MMD correlates with RII at $r{=}0.78$; the full matrix is in Table~\ref{tab:corr}.
...
\textbf{Limitations.} The certificate is output-level by construction: it does not bound leakage in internal representations, which the probe experiments show can persist even for the retraining oracle, and it does not certify parameter-level privacy. The class-level studies are single-run (training cost), so their ranking conclusions are indicative rather than statistically tested; seed stability on the random baseline (10 seeds) and the 7B study (3 subsamples) supports the metric's stability but does not transfer to statistical significance for the class-level rankings. The AG News and 0.5B results are single-run pilot observations reported as transfer evidence rather than full benchmarks. The MI bound requires $\pi_{\min}>0$, and the unified decomposition is exact under the Gaussian signal model, holding to leading order more generally.
...
\textbf{Future work.} Directions include a formal DP--RII connection, extending the author-level TOFU study to the full benchmark suite (forget 90/95/99, Truth Ratio and KLR~\cite{maini2024tofu}) and to MUSE~\cite{shi2024muse}, class-level output channels (e.g.\ author-classification heads) for language models, the multi-layer state-trajectory extension of PSSD (Appendix~\ref{app:pssd}), combining RII with internal representation audits for full white-box certification, ImageNet-scale validation, and---when semantically matched held-out classes are unavailable---constructing MHPR reference subspaces from synthetic or generative reference distributions.
...
Table~\ref{tab:exp_vision} summarizes the visual-benchmark configuration; all runs use a fixed seed (42), no data augmentation, and plain per-channel normalization (CIFAR-10: mean $(0.4914,0.4822,0.4465)$, std $(0.2023,0.1994,0.2010)$; Fashion-MNIST/MNIST: mean $0.2860$, std $0.3530$).
