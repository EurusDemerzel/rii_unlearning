...
In class-level forgetting, the forget and retain rows average over different class distributions. In the standard protocol of Section~\ref{sec:benchmark} the forget class is a \emph{trained} class, so the rows differ because the model's outputs on the forget class differ from those on the retain classes. In two constructions the forget class is instead \emph{unseen} at evaluation time---the leave-one-class-out (LOCO) baseline and the retraining oracle, whose model never saw the forget class---so even under perfect unlearning the rows differ, inflating standard RII.
...
Throughout, ``MIA'' is protocol-specific, denoting \emph{three distinct} measures; to avoid ambiguity, each table states which one it reports: train/test membership-inference accuracy in the random-forgetting baseline (Table~\ref{tab:main}), a forget/retain loss-threshold AUC in the class-level benchmark (Table~\ref{tab:benchmark}), and the forget-set mean next-token NLL on TOFU (Table~\ref{tab:llm}). Where the two leakage axes are contrasted (Sec.~\ref{sec:mia}), ``MIA'' refers to the train/test membership accuracy.
...
Table~\ref{tab:cifar100} repeats the protocol with 20 training classes (forget class~3, held-out $\{20,21,22\}$). The NegGrad hidden failure is \emph{more salient} at scale: its RII ($0.189{\pm}0.016$) exceeds even NoUnlearn ($0.127{\pm}0.026$, a $1.49\times$ gap vs.\ $1.31\times$ on CIFAR-10) while forget accuracy and MIA-loss again improve: the output signature strengthened by gradient ascent is sharper in a larger output space. The known/unknown asymmetry persists (oracle $\rho{=}0.131{\pm}0.002$). MHPR does not separate the oracle ($0.187{\pm}0.157$) from NoUnlearn ($0.152{\pm}0.096$) at this scale---its variance grows with the number of classes---and RII likewise does not separate them (oracle $0.131{\pm}0.002$ vs.\ NoUnlearn $0.127{\pm}0.026$). The robust signal at this scale is RII's separation of the NegGrad hidden failure ($0.189{\pm}0.016$) from NoUnlearn. Six methods are reported; SISA is omitted because its 20-class sharded training is disproportionate at this scale.
...
\textbf{Future work.} Directions include a formal DP--RII connection, extending the author-level TOFU study to the full benchmark suite (forget 90/95/99, Truth Ratio and KLR~\cite{maini2024tofu}) and to long-form book unlearning in the spirit of MUSE~\cite{shi2024muse} (where raw-vocabulary channels are not directly applicable and preliminary evidence indicates that class-level output heads are needed), class-level output channels (e.g.\ author- or book-classification heads) for language models, the multi-layer state-trajectory extension of PSSD (Appendix~\ref{app:pssd}), combining RII with internal representation audits for full white-box certification, ImageNet-scale validation, evaluating recent state-of-the-art unlearning methods such as SCRUB~\cite{kurmanji2023scrub} and SalUn~\cite{fan2024salun} alongside the gradient-based baselines, and---when semantically matched held-out classes are unavailable---constructing MHPR reference subspaces from synthetic or generative reference distributions.

\section*{Declarations}
\begin{itemize}
\item \textbf{Funding.} No specific funding was received for conducting this study.
\item \textbf{Competing interests.} The authors declare that they have no competing interests.
\item \textbf{Ethics approval.} This article does not contain any studies with human participants or animals performed by any of the authors.
\item \textbf{Consent to participate.} Not applicable.
\item \textbf{Consent for publication.} Not applicable.
\item \textbf{Data availability.} The benchmark datasets used (MNIST, Fashion-MNIST, CIFAR-10/100, TOFU) are public; the experiment scripts and generated results are available in the repository below.
\item \textbf{Code availability.} The implementation is available at \url{https://github.com/EurusDemerzel/rii_unlearning}.
\item \textbf{Author contribution.} Both authors contributed to the conceptualization, methodology, experiments, and writing of the manuscript.
\end{itemize}
