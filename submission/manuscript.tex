...
\title{Spectral Certification for Class-Level Machine Unlearning%
\thanks{The implementation is available at \protect\url{https://github.com/EurusDemerzel/rii_unlearning}.}%
}
\titlerunning{Spectral Certification for Class-Level Unlearning}
...
\begin{abstract}
Machine unlearning aims to remove the influence of specified training data from a trained model. Verifying that such removal has been effective remains an open and practically important problem. Membership-inference attacks (MIA) conflate train/test shift with forget-set leakage, and differential-privacy bounds require invasive training, so neither certifies output-level indistinguishability. We introduce a spectral certificate built on a $2\times C$ \emph{empirical output channel matrix}---the mean softmax predictions on the forget and retain sets---and define the \textbf{Residual Irreversibility Index (RII)} $\rho$, which vanishes exactly when the two channel output distributions are indistinguishable. For class-level forgetting, where a known/unknown class asymmetry inflates RII, we extend it to the \textbf{Multi-Held-Out Projection Residual (MHPR)}, which projects the forgotten class mean onto multiple held-out classes and offers finite-sample guarantees and temperature scaling. We prove that $\rho=0$ is equivalent to output-channel independence and derive fine-tuning-leakage, mutual-information, and unified two-axis bounds. Across CIFAR-10, Fashion-MNIST, CIFAR-100, and the TOFU~\cite{maini2024tofu} and MUSE-style book benchmarks with a 7B-parameter language model (LLaMA-2-7B, LoRA), RII ranks methods consistently with known forgetting strength in most settings and remains informative where accuracy and MIA saturate. When generation accuracy saturates at 100\%, accuracy alone cannot rank the checkpoints, whereas RII still orders fine-tuning below no-unlearning. RII also exposes a hidden failure of gradient ascent (its output signature strengthens while accuracy and MIA improve) and a fine-tune rebound that accuracy-based metrics miss. On long-form book unlearning, gradient ascent again leaves the forget output distribution unchanged, forget perplexity ranks retraining as the strongest erasure, and raw-vocabulary channels reflect the books' distinctness rather than the forgetting state, delimiting when RII is informative. Representation-level probes confirm that output-level erasure can coexist with feature-level signatures, motivating a two-layer audit.
...
We address the following research question: \emph{can a lightweight, black-box certificate verify output-level indistinguishability between the forget and retain sets in the class-level regime, where existing metrics are least reliable?} Our answer is a spectral condition on the model's output channel.

We introduce a lightweight, black-box certificate based on a \textbf{spectral condition}: output-space irreversibility reduces to the rank of a $2\times C$ empirical output channel matrix. By \textbf{output distributional irreversibility}, we mean that the forget-set membership indicator $X$ and the model's categorical output $Y$ are conditionally independent given the parameters $\theta'$, i.e., $I(X;Y\mid\theta')=0$. When this holds, the model's output distributions on $D_f$ and $D_r$ are statistically indistinguishable, yielding a rank-one output channel matrix and $\rho=0$.

\textbf{Scope.} Our framework measures irreversibility strictly in the \emph{output space} (the observable predictions), matching black-box GDPR compliance auditing. Parameter-level white-box security is not asserted. We focus on \textbf{class-level forgetting}, a stringent and practically relevant regime, where $D_f$ comprises an entire semantic class; MHPR (Sec.~\ref{sec:mhpr}) specifically addresses the known/unknown class asymmetry that arises there.

The framework builds on a basic observation. The $2\times C$ \textbf{empirical output channel matrix} $\mathbf{M}(\theta')$ has rows $\boldsymbol{\mu}_f$ and $\boldsymbol{\mu}_r$ (mean softmax vectors on $D_f$ and $D_r$). Its singular values $\sigma_1 \ge \sigma_2$ encode residual dependence: $\sigma_2 = 0 \iff \mathrm{rank}(\mathbf{M}) = 1 \iff$ perfect removal.

\textbf{Contributions:}
\begin{enumerate}
\item An output-spectral framework (Sec.~\ref{sec:channel}): a $2\times C$ channel matrix whose rank-one condition yields the tuning-free Residual Irreversibility Index (RII).
\item A proof (Theorem~\ref{thm:perfect}) that, at the \emph{channel} level, $\rho=0$ iff $I(X;Y\mid\theta')=0$, requiring no exponential-family or calibration assumptions; per-sample output distributions are deliberately out of scope.
\item MHPR (Sec.~\ref{sec:mhpr}) for class-level forgetting: a multi-held-out projection that largely resolves the known/unknown class asymmetry (reducing the oracle's residual well below its standard-RII level), with temperature scaling and finite-sample guarantees.
\item Theoretical guarantees (Sec.~\ref{sec:theory}): confidence bounds, an $O(\lambda^2)$ fine-tuning leakage bound, a spectral MI bound, and a unified decomposition of output leakage into two orthogonal axes.
\item An evaluation of RII and MHPR on a unified seven-method benchmark and on the TOFU and MUSE-style book-unlearning benchmarks with a 7B-parameter language model (Sec.~\ref{sec:experiments}), exposing hidden failures (gradient-ascent signature strengthening and fine-tune rebound) that accuracy and MIA miss; on TOFU, RII ranks the unlearned checkpoints whose generation accuracy saturates at 100\%, where accuracy alone provides no ordering, and on long-form book unlearning gradient ascent again leaves the forget output distribution unchanged. We also give an $O(CN)$ audit protocol; a per-sample refinement (PSSD) is provided in Appendix~\ref{app:pssd}.
\end{enumerate}
...
Let $D = \{(x_i, y_i)\}_{i=1}^N$ be a training set, $\theta_0 = \mathcal{A}(D)$ trained parameters, $D_f \subset D$ the forget set, $D_r = D \setminus D_f$ the retain set, with $N_f = |D_f|$, $N_r = |D_r|$. An unlearning algorithm $\mathcal{U}$ produces $\theta' = \mathcal{U}(\theta_0, D_f, D_r)$. We assess the residual influence of $D_f$ through the model's induced output distribution.

We focus on \textbf{class-level unlearning}, where $D_f$ comprises all examples of entire semantic classes. This regime reveals spectral structure that sample-level forgetting obscures.

\subsection{The Empirical Output Channel Matrix}

Let $f_{\theta'}$ be the logit function and $\mathbf{p}(x;\theta') = \mathrm{Softmax}(f_{\theta'}(x)) \in \Delta^{C-1}$. All quantities are computed from the softmax probability vectors rather than hard (argmax) predictions, and the abstract channel variable $Y$ (Theorem~\ref{thm:perfect}) is the class drawn from $p_c(x;\theta')$; the certificate therefore concerns probabilistic output behavior, not label agreement.
...
Definition~\ref{def:rii} (Eq.~\eqref{eq:rho}) defines $\rho$ as a spectral residual energy (the normalized second singular value of the output channel), not a mutual information value. Its relation to mutual information is established by Theorem~\ref{thm:mi_bound}: $I(X;Y\mid\theta')\le O(\rho)$ for $\rho\ll1$, so $\rho$ vanishes exactly when the mutual information vanishes and bounds it from above at small leakage.
...
In class-level forgetting, the forget and retain rows average over structurally different distributions: $\boldsymbol{\mu}_f$ comes from a class the model never saw during training. Even under perfect unlearning, the rows can therefore differ, inflating standard RII.

\subsection{The Leave-One-Class-Out (LOCO) Failure}

Replacing $\boldsymbol{\mu}_r$ with a single held-out unseen class mean (LOCO) fails because different unseen classes produce systematically different prediction profiles: on MNIST (train on $\{1,2,3,4,6,7,8,9\}$, forget class~5, hold out class~0), $\rho_{\mathrm{LOCO}}{\approx}0.38$ exceeds standard RII ${\approx}0.14$, opposite to the intended direction.
...
Unlike the spectral RII $\rho\in[0,0.5]$ (a normalized second singular value of the $2\times C$ channel), $\rho_H$ is the \emph{relative energy of the projection residual} onto the held-out subspace; the different normalizations (spectral energy ratio vs.\ geometric residual norm ratio) account for the different ranges. One caveat is the normalization by $\|\boldsymbol{\mu}_f\|_2^2$: when the model is near-uniform on the forgotten class (e.g., under heavy forgetting or degradation), $\|\boldsymbol{\mu}_f\|_2$ is small and the residual ratio amplifies estimation noise; the unnormalized spectral RII is then the more stable quantity to report.
...
When softmax outputs are overly concentrated (accuracy $\lesssim 90\%$), the held-out class mean vectors can become nearly collinear, collapsing the subspace $\mathcal{S}_K$ to a one-dimensional manifold; we observed this collapse in one Fashion-MNIST run ($\rho_H\approx1$), but across three seeds it is seed-dependent rather than systematic (Section~\ref{sec:benchmark}).

To mitigate output concentration, we apply temperature scaling $\mathbf{p}_T(x) = \mathrm{Softmax}(f_{\theta'}(x)/T)$; as $T\to\infty$, $\mathbf{p}_T \to \mathbf{1}/C$ (uniform) and the held-out subspace becomes well-conditioned. We select the smallest $T\in\{1,2,5,10,50,100\}$ whose condition number satisfies $\kappa(\mathbf{H}(T)) < 10$. On Fashion-MNIST this selects $T=50$ ($\rho_H = 0.021$), on CIFAR-10 $T=5$ ($\rho_H = 0.025$). For high-accuracy models (MNIST, $\sim97.5\%$), $T=1$ suffices; the ranking-preservation property underlying this selection is proved in Proposition~\ref{prop:rank_preserve} (Appendix~\ref{app:temp}). A sweep over $T$ confirms that the criterion does not sacrifice signal: on both Fashion-MNIST and CIFAR-10 the softmax RII peaks near $T{=}5$ and falls by more than an order of magnitude at $T{=}10$, approaching zero as $T\to\infty$, so among the temperatures that satisfy the conditioning constraint the smallest admissible one is also the most discriminative.

\subsection{Bridging Logit-State and Softmax RII}
\label{sec:logit_bridge}

Quantizing logits into $m$ states ($k$-means on training logits) yields state distributions $\boldsymbol{\pi}_c\in\Delta^{m-1}$. Throughout, $\chi^2(P\|Q)\coloneqq\sum_i (P_i-Q_i)^2/Q_i$ denotes the $\chi^2$ divergence. The state-space RII $\rho_{\mathcal{S}}$ approximates the softmax RII (Theorem~\ref{thm:state_bridge}, Appendix~\ref{app:bridge}); in state space, MHPR admits a clean $\chi^2$ bound:

\begin{theorem}[State-Space MHPR via $\chi^2$ Control]
\label{thm:state_mhpr}
Let $\boldsymbol{\pi}_f$ be the forget state distribution and $\bar{\boldsymbol{\pi}}_H = \frac{1}{K}\sum_k \boldsymbol{\pi}_{h_k}$. If $\chi^2(\boldsymbol{\pi}_f \| \bar{\boldsymbol{\pi}}_H) \le \tau^2$ and $\|\boldsymbol{\pi}_f\|_2^2 \ge \gamma > 0$, then
\begin{equation}
\rho_{H,\mathcal{S}} \le \frac{\tau^2}{\gamma}.
\label{eq:state_mhpr}
\end{equation}
Thus MHPR is a consistent proxy for output-level indistinguishability in state space.
\end{theorem}

Empirical validation (Appendix~\ref{app:chi2}) confirms the bound of Eq.~\eqref{eq:state_mhpr}, i.e., $\rho_{H,\mathcal{S}} \le \chi^2/\|\boldsymbol{\pi}_f\|_2^2$, for all gradient-ascent steps. Additional state-space results (quantization error, Lemma~\ref{lem:quantization}; spectrum bridge, Theorem~\ref{thm:state_bridge}; MHPR deviation rate, Theorem~\ref{thm:softmax_deviation}) are deferred to Appendix~\ref{app:bridge}.
...
\subsection{Assumptions and Practical Validity}
\label{sec:assumptions}

Table~\ref{tab:assumptions} lists the assumptions behind each theoretical result and their status in the experimental settings of this paper; we then examine the most consequential of these assumptions.

\begin{table}[t]
\centering
\footnotesize
\caption{Assumptions behind each theoretical result and their practical validity in our experimental settings.}\label{tab:assumptions}
\setlength{\tabcolsep}{2pt}
\begin{tabular}{p{2.0cm} p{4.9cm} p{4.8cm}}
\toprule
Result & Key assumption & Validity in our settings \\
\midrule
Thm.~\ref{thm:perfect} & $Y$ is a class drawn from the softmax distribution $p_c(x;\theta')$, not an argmax label & Holds by construction: all quantities are computed on softmax probability vectors (Sec.~\ref{sec:channel}) \\
Prop.~\ref{prop:confidence} & Softmax vectors sub-Gaussian with variance proxy $\nu^2$; $\sigma_1>0$ & $p_c\in[0,1]$ is bounded, hence sub-Gaussian with $\nu=1$ (below); $\sigma_1>0$ since rows are nonzero probability vectors \\
Thm.~\ref{thm:finetune} & $L$-Lipschitz gradient and softmax; small-step condition $\lambda\|\mathbf{g}\|_2=O(1)$ & Bounded in practice, especially under LoRA (small parameter space); $\lambda{\sim}10^{-5}$ satisfies the condition \\
Thm.~\ref{thm:mi_bound} & $\pi_{\min}=\min_y(\boldsymbol{\mu}_r)_y>0$; equal row norms (relaxed asymptotically) & Empirical channels have $\pi_{\min}\ge 1/N_r>0$ (below); equal-norm is relaxed asymptotically (proof in Appendix~\ref{sec:appendix}) \\
Thm.~\ref{thm:unified} & Gaussian signal model $R=\alpha X+\beta Y+N$; small $\alpha,\eta$ & An idealization; exact under the model, leading-order otherwise (Remark~\ref{rem:unified_status}) \\
MHPR props. & Held-out subspace well-conditioned ($\sigma_{\min}(\mathbf{H}^*)>0$); finite-sample concentration & Holds with $K\ge2$ and temperature scaling when the subspace would otherwise collapse (Sec.~\ref{sec:mhpr}) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Validity in practice.} (i) \emph{Sub-Gaussian softmax.} Every coordinate of the softmax vector lies in $[0,1]$, so it is bounded and hence sub-Gaussian: by Hoeffding's lemma a variable supported on $[0,1]$ is sub-Gaussian with variance proxy $1/4$, and we use $\nu=1$ as a conservative upper bound. Proposition~\ref{prop:confidence} therefore holds with explicit constants and no unknown distributional parameters. (ii) \emph{$\pi_{\min}>0$.} In every experiment each class appears at least once on the retain set, so the empirical $\pi_{\min}\ge 1/N_r>0$; for population channels with a large number of classes the truncated-support argument of Remark~\ref{rem:pi_min} applies. (iii) \emph{$L$-Lipschitz gradient and softmax.} The softmax map is $1$-Lipschitz in its logit input, and the gradient Lipschitz constant is the spectral norm of the Hessian of the loss, which is bounded in the (small) parameter spaces considered here---and especially for LoRA adapters, which update a few low-rank factors. We do not rely on a specific value: the bound of Theorem~\ref{thm:finetune} is the qualitative statement $\rho=O(\lambda^2)$ for small steps. (iv) \emph{Cross-covariance under class-level forgetting.} Lemma~\ref{lem:cross_cov} (Appendix~\ref{app:finetune}) relies on logit gradients of different classes being near-orthogonal at a well-trained classifier, which holds when the classifier is confident ($p_f(c_f)\approx1$); the appendix gives the derivation.

\subsection{Pseudocode}
\label{sec:pseudocode}

For reproducibility we give the computation of RII, MHPR, and the recommended audit protocol as pseudocode; the implementation is available at \url{https://github.com/EurusDemerzel/rii_unlearning}.
...
The following result concerns the \emph{channel}---the conditional emission law of the categorical output $Y$---not the per-sample distribution of softmax vectors; the scope of this distinction is discussed in Remark~\ref{rem:channel}.

\begin{theorem}[Output Distributional Irreversibility]
\label{thm:perfect}
Let $X\in\{0,1\}$ be the forget-set membership indicator ($X{=}1$ for forget, $X{=}0$ for retain, abbreviated below as $X{=}f$ and $X{=}r$), and let $Y\in\{1,\dots,C\}$ denote the model's categorical output under $\theta'$: for input $x$, the channel emits $Y{=}c$ with probability $p_c(x;\theta')$, so that $P(Y{=}c\mid X{=}f)=(\boldsymbol{\mu}_f)_c$ and $P(Y{=}c\mid X{=}r)=(\boldsymbol{\mu}_r)_c$ by the law of total probability (Remark~\ref{rem:channel}). Then
\begin{equation}
...
Let $\rho^*$ denote the population RII of the true channel matrix $\mathbf{M}^*$, and $\hat{\rho}$ its estimate from $N_f, N_r$ samples. For sub-Gaussian softmax vectors with variance proxy $\nu^2$,
\begin{equation}
...
\label{thm:finetune}
For one-step gradient ascent $\theta' = \theta_0 + \lambda\mathbf{g}$ with $\mathbf{g}=\nabla_\theta\mathcal{L}(\theta_0, D_f)$, $L$-Lipschitz gradient and $L$-Lipschitz softmax map, and step size $\lambda$ satisfying the small-step condition $\lambda\|\mathbf{g}\|_2=O(1)$,
\begin{equation}
...
\end{equation}
where $\boldsymbol{\Sigma}_f \coloneqq \operatorname{Cov}_{x\sim D_f}(\nabla_\theta f_{\theta_0}(x))$ is the gradient covariance of the logit map on the forget set and $\sigma_1$ is the leading singular value of the channel at $\theta_0$. No normalization of $\mathbf{g}$ is assumed; its scale enters through the small-step condition and through $\operatorname{Tr}(\boldsymbol{\Sigma}_f)$, which lower-bounds the mean squared gradient norm ($\operatorname{Tr}(\boldsymbol{\Sigma}_f)=\mathbb{E}\|\nabla_\theta f-\mathbb{E}\nabla_\theta f\|_2^2\le\mathbb{E}\|\nabla_\theta f\|_2^2$). Hence $\rho = O(\lambda^2)$ for small steps.
\end{theorem}

\begin{remark}[Scope of Theorem~\ref{thm:finetune}]
The bound is local: it holds while the trajectory stays in the $L$-Lipschitz regime of $\theta_0$, which the small-step condition enforces. Equivalently, one may normalize the ascent direction and absorb $1/\|\mathbf{g}\|_2$ into the definition of $\lambda$; the statement is unchanged up to a rescaling of the effective step. At the learning rates used in practice ($\lambda{\sim}10^{-5}$ with $\|\mathbf{g}\|_2$ of order one), the condition is readily satisfied.
\end{remark}
...
Consequently, the class-level bound preserves the $O(\lambda^2)$ rate of Eq.~\eqref{eq:finetune_bound}: its $O(\lambda^3)$ term absorbs the cross-covariance contribution (Appendix~\ref{app:finetune}), and the rate is maintained because $\|\boldsymbol{\Sigma}_{fr}\|_F = O(\lambda)$ adds only a higher-order correction, negligible at practical learning rates ($\lambda{\sim}10^{-5}$).


\begin{theorem}[Spectral $\chi^2$–MI Bound]
\label{thm:mi_bound}
For binary membership $X$ and predicted class $Y$ under $\theta'$, when the channel rows have equal norm, $\|\boldsymbol{\mu}_f\|_2=\|\boldsymbol{\mu}_r\|_2$ (a condition relaxed asymptotically below),
\begin{equation}
I(X; Y \mid \theta') \le \frac{\sigma_1^2}{\pi_{\min}}\cdot\frac{\rho}{1-\rho},
\end{equation}
where $\pi_{\min} \coloneqq \min_y (\boldsymbol{\mu}_r)_y > 0$ and $(\boldsymbol{\mu}_r)_y=P(Y{=}y\mid X{=}r)$. For $\rho\ll 1$, $I \le O(\rho)$; the bound vanishes as $\rho\to0$, and holds asymptotically without the equal-norm condition (proof in Appendix~\ref{sec:appendix}).
\end{theorem}

\begin{remark}[On $\pi_{\min}>0$]
\label{rem:pi_min}
For the empirical channel, $(\boldsymbol{\mu}_r)_y$ is the relative frequency of class $y$ on the retain set, so $\pi_{\min}>0$ holds whenever every class appears at least once---the case in all experiments here. For population channels with a large number of classes, where some classes may carry arbitrarily small probability, the bound can be applied to the truncated support $\mathcal{Y}_\epsilon=\{y:(\boldsymbol{\mu}_r)_y>\epsilon\}$, paying a multiplicative $O(1/\epsilon)$ factor; the same truncation appears in the confidence bounds (Proposition~\ref{prop:confidence}).
\end{remark}
...
\label{lem:fisher}
Assume $f_Z\in C^2$ and that $f_Z$ and its derivatives vanish sufficiently fast at infinity for the integration by parts below. For $R = \alpha X + Z$ with $X \perp Z$, as $\alpha \to 0$:
...
\begin{remark}[Status of Theorem~\ref{thm:unified}]
\label{rem:unified_status}
The decomposition is exact under the Gaussian signal model and holds to leading order in $\alpha^2+\eta^2$ under the regularity of Lemma~\ref{lem:fisher} (small $\alpha$, small spectral deviation); we state it as a theorem for readability, with a proof sketch in Appendix~\ref{app:unified} and the full $\chi^2$ control in Appendix~\ref{app:bridge_proofs}.
\end{remark}
...
\textbf{Datasets and models:} MNIST (60K, 10 classes), Fashion-MNIST (60K, 10), CIFAR-10 (50K, 10), CIFAR-100 (50K, 100; 20-class subset in the class-level protocol). Architectures: SimpleMLP (784$\to$128$\to$10), SmallCNN (3 conv + 2 fc), ResNet-18, DeepMLP (for overfitting). Training: Adam, lr $10^{-3}$, batch 64, 10 epochs. The language-model study (Section~\ref{sec:llm}) uses the TOFU~\cite{maini2024tofu} author-level benchmark on LLaMA-2-7B with LoRA-based PEFT.

\textbf{Unlearning methods:} NoUnlearn (no removal), Retrain (gold standard), SISA~\cite{bourtoule2021machine} ($S{=}5$ shards), NegGrad (gradient ascent on $D_f$ only), FineTune (gradient ascent on $D_f$ followed by descent on $D_r$), KED (knowledge erosion via KL-to-uniform on $D_f$), and BadTeacher (wrong-label training on $D_f$).

\textbf{Benchmark protocol.} In Section~\ref{sec:benchmark} we adopt a standard class-level protocol on CIFAR-10: the model is trained on classes $\{0,\dots,6\}$, class~3 (cat) is the forget set, and classes $\{7,8,9\}$ are held out as unseen MHPR references ($K{=}3$). Fashion-MNIST uses the identical protocol: seven training classes $\{0,\dots,6\}$, forget class~3 (Dress), and held-out classes $\{7,8,9\}$ (Sneaker, Bag, Ankle boot). Held-out classes are the remaining classes after the training split in both cases---chosen for availability rather than semantic similarity; the effect of this choice is discussed in Section~\ref{sec:benchmark} (Caveats~(2)). Models are trained without data augmentation for determinism, and every unlearning method is evaluated with the \emph{same} battery of metrics.

The abbreviation ``MIA'' denotes three distinct protocols across the paper, and each table states its own: train/test membership-inference accuracy in the random-forgetting baseline (Table~\ref{tab:main}); a forget/retain loss-threshold AUC in the class-level benchmark (Table~\ref{tab:benchmark}); and the forget-set mean next-token NLL on TOFU (Table~\ref{tab:llm}). Where the two leakage axes are contrasted (Sec.~\ref{sec:mia}), ``MIA'' refers to the train/test membership accuracy.

Across the experiments below, standard training induces near-rank-one output channels ($\rho<5\times10^{-3}$), making RII a stable, tuning-free quantity: random sample-level forgetting is trivially indistinguishable, whereas class-level forgetting is not (Sec.~\ref{sec:benchmark}). Under class-level forgetting, RII and MHPR rank unlearning methods consistently with forgetting strength and expose methods that leave---or even strengthen---an output signature that accuracy and MIA miss; on a 7B-parameter language model with the TOFU benchmark (Section~\ref{sec:llm}) the same ordering holds even when all unlearned checkpoints saturate generation accuracy at 100\%. Finally, RII and MIA measure overlapping but distinct leakage axes ($r{=}0.57$), so a complete audit should report both.
...
\centering
\small
\caption{Rank-one baseline: RII under standard training (5\% random forget; all rows use NoUnlearn). MIA is the train/test membership-inference accuracy (Sec.~\ref{sec:mia}), not a forget/retain loss-threshold AUC.}\label{tab:main}
\setlength{\tabcolsep}{2pt}
...
\bottomrule
\end{tabular}
\par\vspace{2pt}\footnotesize MIA is the mean next-token NLL of the forget set, \emph{not} an AUC; higher values indicate stronger forgetting.
\end{table}

All three unlearned checkpoints (and the untouched model) answer all forget questions (Forget$=$100\%; many TOFU answers are recoverable from the question itself), so generation accuracy cannot separate them. RII, by contrast, ranks FineTune below NoUnlearn ($9.73{\times}10^{-2}$ vs.\ $1.16{\times}10^{-1}$, $-16\%$), the same ordering as the vision benchmarks, and separates the (lightweight, 60-step) retraining oracle ($8.01{\times}10^{-2}$) from the untouched model by $1.45\times$. The oracle is additionally separated by generation accuracy (80\% vs.\ 100\%) and by its much larger MIA (2.163 vs.\ 1.348); the claim here is narrower: among the checkpoints whose generation accuracy is saturated at 100\%, RII provides an ordering that accuracy cannot. We do not claim to separate NegGrad from NoUnlearn on RII alone: their full-set gap ($1.7{\times}10^{-3}$) falls inside the evaluation noise, and their subsampled means coincide ($0.127\pm0.002$ for both, below).

NegGrad's RII ($1.17{\times}10^{-1}$) is statistically indistinguishable from NoUnlearn's ($1.16{\times}10^{-1}$) under our evaluation protocol, unlike FineTune's 16\% decrease; its MIA (1.352) likewise matches NoUnlearn (1.348). Ascent thus produces no measurable output-space erasure here, and the strength sweep below shows the forget output signature \emph{strengthens} as ascent intensifies (rising to $1.20{\times}10^{-1}$ at $3\times$ strength before the model breaks down), the 7B analogue of the hidden failure documented on CIFAR-10/100 (Section~\ref{sec:benchmark}).

Continued LoRA training on retain answers moves the forget and retain output distributions together ($\rho{=}9.73{\times}10^{-2}$, $-16\%$) and achieves the lowest MIA (0.852; the model remains most fluent on the forget set), consistent with the vision results; as in Section~\ref{sec:rebound}, a reduced RII is necessary but not sufficient for true erasure, since forget accuracy remains 100\%.

To gauge evaluation stability, we repeat the evaluation over three random subsamples of the evaluation set; RII standard deviations are below $3.5{\times}10^{-3}$ ($<$3\% relative) for every method, and the ranking Retrain $<$ \allowbreak FineTune $<$ \allowbreak NoUnlearn ${\approx}$ \allowbreak NegGrad is unchanged across all seeds, with the gap between NegGrad and NoUnlearn ($1.7{\times}10^{-3}$) inside the noise. Computed on the subsampled sets (hence not directly comparable to the full-set values of Table~\ref{tab:llm}), the subsampled means are $0.088\pm0.003$ (oracle), $0.107\pm0.002$ (FineTune), and $0.127\pm0.002$ for both NoUnlearn and NegGrad.

On the small Qwen2.5-0.5B pilot (Section~\ref{sec:benchmark}) vocabulary-level channels could not separate the oracle from the untouched model, because at 0.5B scale both answer sets induce statistically similar text distributions. At 7B scale the separation is significant ($1.45\times$, with no overlap of the three subsampled intervals): the larger model encodes author identity more strongly in next-token distributions, so even raw-vocabulary channels carry the class-level signal. Channel \emph{semantics} therefore determine discriminative power (class-level heads remain the recommended audit), and model capacity determines whether raw channels suffice (cf.\ Remark~\ref{rem:channel}).

We also sweep the gradient-ascent strength (steps $\times$ learning rate), from the baseline $10\times 2{\times}10^{-5}$ to $30\times 10^{-4}$ ($3\times$ steps, $5\times$ lr) and $60\times 2{\times}10^{-4}$ ($6\times$ steps, $10\times$ lr), each time starting from a fresh zero LoRA. At the moderate strength, RII \emph{rises} further to $1.20{\times}10^{-1}$ (above NoUnlearn's $1.16{\times}10^{-1}$) while forget/retain accuracy stay at 100\%: the hidden failure does not saturate: stronger ascent sharpens the forget output signature. At the strongest setting the model begins to break down: MIA jumps to 5.25 (from 1.35) and the ascent loss diverges to 6.5, while RII falls back to $1.01{\times}10^{-1}$, yet generation accuracy is still 100\%, so accuracy alone still cannot detect the collapse. RII decreases here \emph{because the output distribution degenerates into near-random noise}, which coincidentally makes the two sets look statistically similar again; the collapse is instead flagged by MIA. RII is therefore bidirectional: it rises as hidden-failure signatures strengthen and falls as the output distribution itself degenerates, so a low RII should be read alongside utility metrics (Retain accuracy, MIA) to distinguish true erasure from model collapse, in line with the two-axis audit of Section~\ref{sec:mia}.

\subsection{RII on MUSE-Style Long-Form Book Unlearning}
\label{sec:muse}

TOFU is question--answer based (short prompts on a shared format). To test the certificate on long-form knowledge unlearning in the spirit of MUSE~\cite{shi2024muse}---a model first memorizes a book and is then asked to forget it---we use public-domain books (Project Gutenberg). The forget set is \emph{Alice's Adventures in Wonderland} (Carroll, 1865); the retain set is \emph{Through the Looking-Glass} (Carroll, 1871), a same-author sequel whose style is comparable to the forget text (a different-author variant, \emph{The Wonderful Wizard of Oz}, gives the same qualitative picture; see below). The teacher is base LLaMA-2-7B plus 200 LoRA steps on the forget book; the four checkpoints are otherwise identical to Section~\ref{sec:llm}: NoUnlearn (teacher as-is), FineTune (60 LoRA steps on the retain book), NegGrad (15 LoRA gradient-ascent steps on the forget book), and Retrain (base plus LoRA on the retain book only). All texts are chunked with the LLaMA-2 tokenizer; held-out chunks (25 per set) are disjoint from the training chunks. RII is computed on the $2\times V$ channel matrix as in Section~\ref{sec:llm}; forget/retain perplexity is $\exp$ of the mean next-token NLL, and MIA is the forget-set mean NLL.

\begin{table}[t]
\centering
\small
\caption{MUSE-style long-form book unlearning on LLaMA-2-7B (forget: \emph{Alice's Adventures in Wonderland}; retain: \emph{Through the Looking-Glass}; LoRA-based; Retrain is a lightweight 60-step oracle).}\label{tab:muse}
\setlength{\tabcolsep}{2pt}
\begin{tabular}{l c c c c}
\toprule
Method & ForgetPPL & RetainPPL & RII & MIA \\
\midrule
NoUnlearn & 1.97 & 3.39 & $1.18{\times}10^{-2}$ & 0.676 \\
FineTune & 1.96 & $\mathbf{3.22}$ & $1.05{\times}10^{-2}$ & 0.674 \\
NegGrad & 1.97 & 3.40 & $1.18{\times}10^{-2}$ & 0.676 \\
Retrain (oracle) & $\mathbf{2.00}$ & 3.29 & $1.26{\times}10^{-2}$ & $\mathbf{0.691}$ \\
\bottomrule
\end{tabular}
\par\vspace{2pt}\footnotesize ForgetPPL/RetainPPL are $\exp$ of the mean next-token NLL on held-out chunks; higher forget perplexity indicates stronger forgetting. MIA is the forget-set mean next-token NLL (higher is stronger forgetting). Bold marks the most-forgotten checkpoint per column.
\end{table}

Table~\ref{tab:muse} reports the results. The teacher does learn the forget book: forget perplexity drops from 2.11 (base) to 1.97, while retain perplexity stays near the base level (3.52 to 3.39). The headroom is small because these public-domain books are already present in the base model's pretraining corpus---a limitation of using real books. Under unlearning, forget perplexity ranks Retrain as the strongest erasure (2.00, the highest value), followed by FineTune (1.96) and NoUnlearn (1.97); MIA agrees (0.691 for Retrain vs.\ 0.674--0.676 for the others). NegGrad is the notable result: after 15 ascent steps---and in a separate run after 50---its forget perplexity, MIA, and RII are indistinguishable from NoUnlearn's to four significant figures (1.97, 0.676, $1.18{\times}10^{-2}$). Gradient ascent therefore produces no measurable output-space erasure on long-form text either, the book-domain analogue of the TOFU result (Section~\ref{sec:llm}). FineTune instead lowers retain perplexity (3.39 to 3.22, $-5\%$) while leaving forget perplexity essentially unchanged (1.97 to 1.96), again matching the TOFU pattern in which continued fine-tuning preserves forget fluency.

RII on raw-vocabulary channels does \emph{not} separate these checkpoints. All methods fall in $[1.05{\times}10^{-2}, 1.26{\times}10^{-2}]$, and the untouched base model's reference value ($1.34{\times}10^{-2}$) is above most of them; Retrain, the strongest erasure by perplexity, has the \emph{highest} RII ($1.26{\times}10^{-2}$). The reason is structural: forget and retain are two different books, so their next-token distributions differ regardless of what the model remembers---RII measures the books' inherent distinctness rather than the forgetting state. A same-author retain halves the baseline (from $2.58{\times}10^{-2}$ with the different-author Oz retain to $1.34{\times}10^{-2}$ with Looking-Glass), but the raw-vocabulary channel still cannot rank the methods. This is the long-text regime predicted by Remark~\ref{rem:channel}: channel semantics determine discriminative power, and raw-vocabulary channels are informative only when the forget and retain sets induce comparable output distributions (as in TOFU's QA pairs); for distinct books, class-level output heads (e.g.\ a book-classification head) are the recommended audit. We therefore report the book study as evidence of where RII applies and where it requires complementing; on both retain variants NegGrad stays indistinguishable from NoUnlearn and Retrain attains the highest forget perplexity.
...
\centering
\small
\caption{Class-level unlearning benchmark on CIFAR-10 (forget cat, $K{=}3$ held-out); mean $\pm$ std over three seeds.}\label{tab:benchmark}
...
\bottomrule
\end{tabular}
\par\vspace{2pt}\footnotesize MIA-loss is a loss-threshold AUC; the value $0.000$ for Retrain is expected (the oracle never trained on the forget class, giving a perfect reverse ranking; see Sec.~\ref{sec:benchmark}). Representation MMD and residual-probe AUC are reported in the text (Sec.~\ref{sec:benchmark}).
\end{table}

Even the retraining oracle (mathematically perfect unlearning) has nonzero RII, $\rho{=}0.113{\pm}0.005$: because cat is now an \emph{unseen} class, its output distribution differs from the retain classes, so standard RII cannot certify the oracle as perfect under class-level forgetting. This known/unknown asymmetry is what MHPR addresses, narrowing the gap to NoUnlearn ($0.468$ vs $0.802$).

Gradient ascent (NegGrad) is the \emph{only} method whose RII \emph{rises} with unlearning ($0.249{\pm}0.002$ vs $0.190{\pm}0.036$ for NoUnlearn, Table~\ref{tab:benchmark}), even though forget accuracy and MIA-loss improve. Ascent drives the forget class toward confident, systematically wrong predictions, concentrating the output signature and making the channel \emph{more} distinguishable; among the metrics we report, RII is the only one to expose this failure, since accuracy and MIA-loss both improve. Together with the fine-tune rebound (Section~\ref{sec:rebound}), output-space erasure is not monotone in gradient-based forgetting strength.

\begin{table}[t]
\centering
\small
\caption{Pearson correlations of evaluation metrics across the seven unlearning methods (CIFAR-10; each method is its three-seed mean, so the seven method-level points are treated as indicative).}
\label{tab:corr}
...
RII and MHPR are strongly consistent ($r{=}0.90$) and track forget accuracy ($r{=}0.75$ and $0.83$); the RII--forget-accuracy correlation is driven by the Retrain endpoint (forget accuracy $0\%$), and among the non-oracle methods the two metrics diverge systematically: NegGrad's forget accuracy \emph{falls} (from $85.2\%$ to $63.8\%$) while its RII \emph{rises} (from $0.190$ to $0.249$), a failure an accuracy proxy would miss. RII correlates moderately with MIA-loss ($r{=}0.57$) and disagrees at the top (NegGrad is worst under RII but second-worst under MIA-loss), supporting the orthogonal-axes view (Section~\ref{sec:mia}). Representation MMD correlates with RII at $r{=}0.77$; the full matrix is in Table~\ref{tab:corr}.

A residual linear probe separating forget-class from held-out features succeeds for \emph{all} methods ($\mathrm{AUC}\approx0.94$--$0.98$), including the oracle ($0.956$): output-level erasure coexists with representation-level signatures, consistent with~\cite{cosma2026ruler,yong2026erased} and motivating the two-layer audit (Section~\ref{sec:mia}).

Several points should be kept in mind when interpreting these results. (1) SISA's low retain accuracy ($70.4\%$) reflects a light training budget ($5$ shards $\times$ $2$ slices), so its near-zero spectral scores are a utility artifact and we exclude it from the ranking conclusions: a degenerating model can drive RII toward zero (as with breakdown in Sections~\ref{sec:llm} and \ref{sec:benchmark}), so a low RII should be read together with retain accuracy. (2) Absolute MHPR on CIFAR-10 is higher than on MNIST because $\{7,8,9\}$ are not semantically close to cat; MHPR is most discriminative when the held-out reference is representative. (3) MIA-loss AUC is below $0.5$ because cat is harder than the retain classes ($85.2\%$ vs $92.6\%$ forget/retain accuracy), so the loss threshold separates them in the ``wrong'' direction; relative ordering is meaningful. The extreme value $0.000$ for the Retrain oracle is expected rather than anomalous: since the oracle never trained on cat, its loss on cat exceeds every retain sample, so the loss-based MIA assigns every forget sample to the non-member class, a perfect reverse ranking (AUC${\to}0$) that is itself distributional evidence of successful erasure. (4) The class-level benchmarks below are over three seeds, reported as mean $\pm$ std; they support the metric's stability and the qualitative rankings, but with $n{=}3$ they do not by themselves establish statistical significance for close pairs (e.g., NegGrad vs NoUnlearn under RII). RII's seed stability is further corroborated by the random baseline (10 seeds, SD$<3.5{\times}10^{-4}$) and the 7B study (3 subsamples, $<$3\% relative).

Table~\ref{tab:cross} repeats the protocol on Fashion-MNIST with an MLP. RII remains discriminative: the NegGrad failure persists ($\rho{=}0.169{\pm}0.004$ vs $0.166{\pm}0.004$), the oracle keeps nonzero RII ($0.089{\pm}0.012$), and KED attains the lowest RII ($0.063{\pm}0.006$). Unlike the single-run baseline analyzed earlier, MHPR is here discriminative without temperature scaling across all three seeds: the oracle ($0.429{\pm}0.137$) is ranked \emph{below} NoUnlearn ($0.749{\pm}0.244$), and the low-accuracy subspace collapse observed in a single earlier run (Section~\ref{sec:mhpr}) is seed-dependent rather than systematic; temperature scaling remains the recommended safeguard for low-accuracy models in general. Representation probes again saturate near $\mathrm{AUC}{=}1$.

\begin{table}[t]
\centering
\small
\caption{Cross-dataset benchmark with an identical protocol: RII/MHPR/MIA on CIFAR-10 (SmallCNN) and Fashion-MNIST (MLP); mean $\pm$ std over three seeds.}\label{tab:cross}
...
Table~\ref{tab:cifar100} repeats the protocol with 20 training classes (forget class~3, held-out $\{20,21,22\}$). The NegGrad hidden failure is \emph{more salient} at scale: its RII ($0.189{\pm}0.016$) exceeds even NoUnlearn ($0.127{\pm}0.026$, a $1.49\times$ gap vs.\ $1.31\times$ on CIFAR-10) while forget accuracy and MIA-loss again improve: the output signature strengthened by gradient ascent is sharper in a larger output space. The known/unknown asymmetry persists (oracle $\rho{=}0.131{\pm}0.002$). MHPR, however, does not separate the oracle ($0.187{\pm}0.157$) from NoUnlearn ($0.152{\pm}0.096$) at this scale---its variance grows with the number of classes---so the spectral RII separation is the more robust signal here. Six methods are reported; SISA is omitted because its 20-class sharded training is disproportionate at this scale.

\begin{table}[t]
\centering
\small
\caption{Class-level unlearning benchmark on CIFAR-100 (20 train classes, forget class~3, $K{=}3$ held-out $\{20,21,22\}$); mean $\pm$ std over three seeds.}\label{tab:cifar100}
\setlength{\tabcolsep}{2pt}
\begin{tabular}{l c c c c c}
\toprule
Method & Ret.(\%) & For.(\%) & $\rho$ & MHPR & MIA-loss \\
...
The benchmarks above are visual. To test transfer across modalities, we repeat the protocol on text. On AG News (four classes, forget set $=$ World news), a DistilBERT classifier~\cite{sanh2020distilbert} fine-tuned for two epochs shows the same ordering as in vision: RII separates the untouched model ($\rho{=}0.267$) from the retraining oracle ($\rho{=}0.090$), a $2.96{\times}$ gap consistent with the CIFAR-10 results. These text results are single-run pilot observations, reported as transfer evidence rather than a full cross-modal benchmark, since we do not repeat them over seeds. Gradient ascent behaves differently here: it collapses the model outright (retain accuracy falls to $37.5\%$, $\rho{\approx}0$), an explicit rather than hidden failure. A low RII is thus necessary but not sufficient evidence of erasure: it should be read together with retain accuracy to tell true removal from model collapse, in line with the two-axis view of Section~\ref{sec:mia}. The same picture emerges on a small language model (Qwen2.5-0.5B~\cite{qwen2024qwen2}) under the author-level TOFU~\cite{maini2024tofu} protocol: fine-tuning on the retain set after exposure leaves the forget answers intact (accuracy unchanged), reproducing the rebound of Section~\ref{sec:rebound} in a decoder-only model, whereas gradient ascent again destroys the model ($\rho{\to}0$, loss grows by an order of magnitude). This small-model pilot also exposes a genuine boundary: at 0.5B scale, vocabulary-level output channels cannot separate the oracle from the untouched model ($\rho{=}0.080$ vs.\ $0.083$), since both answer sets induce statistically similar text distributions. Section~\ref{sec:llm} shows that at 7B scale the same vocabulary channels become discriminative. The certificate's discriminative power therefore lies in the \emph{class semantics} of the output channels, and model capacity determines whether raw vocabulary channels suffice (cf.\ Remark~\ref{rem:channel}); language-model audits should prefer class-level channels (e.g.\ a classification head over authors).
...
Table~\ref{tab:dynamic_range} shows $\rho$ for class forgetting is \textbf{152$\times$ larger} than random forgetting, demonstrating RII is not degenerate. RII decreases with gradient ascent steps after an initial transient increase at the first step (Table~\ref{tab:gradient_sweep}), consistent with the $O(\lambda^2)$ bound of Eq.~\eqref{eq:finetune_bound} (Theorem~\ref{thm:finetune}), where $\lambda$ is the ascent step size. These controlled sweeps (Tables~\ref{tab:dynamic_range}, \ref{tab:gradient_sweep}, and the rebound study of Table~\ref{tab:rebound}) use separately trained baselines from the main benchmark of Section~\ref{sec:benchmark}, so the NoUnlearn reference differs there ($\rho{=}0.183$ vs.\ $0.190{\pm}0.036$ in Table~\ref{tab:benchmark}); the within-table relative patterns are what we compare across these studies.
...
Cross-dataset, adaptive temperature scaling extends MHPR to low-accuracy regimes in which the held-out subspace can collapse (a seed-dependent failure on Fashion-MNIST, Section~\ref{sec:benchmark}). On Fashion-MNIST ($88.7\%$ accuracy), $T{=}50$ reduces $\rho_H$ from $\sim1$ to $0.021$ in the collapsed run; on CIFAR-10 ($75\%$), $T{=}5$ gives $\rho_H{=}0.025$. These compare favorably to the class-level RII baselines ($0.166{\pm}0.004$ and $0.190{\pm}0.036$ for Fashion-MNIST and CIFAR-10, Table~\ref{tab:cross}).
...
Section~\ref{sec:benchmark} ranked gradient ascent as the weakest class-level unlearning method. A practical question is whether that ranking persists under the common ascent-then-fine-tune recipe used in practice.
...
\paragraph{Cross-dataset and sensitivity.} $\rho\le2\times10^{-3}$ for $C{=}10$--$100$ (MNIST, CIFAR-10/100), confirming the rank-one property is structural. Under deliberate overfitting (Table~\ref{tab:sensitivity}, Sec.~\ref{sec:sensitivity}), $\rho$ increases tenfold (MNIST 60K$\to$5K, 50 epochs: $9.8\times10^{-4}\to1.24\times10^{-2}$). The controlled $\alpha$-perturbation confirms orthogonal axes: $\alpha{=}0.20$ raises RII $9\times$ while MIA rises by $6.8$ percentage points. ResNet-18 on CIFAR-100 keeps $\rho{=}1.41\times10^{-3}$ (Table~\ref{tab:resnet18}), confirming architecture independence.
...
Under deliberate overfitting (Table~\ref{tab:sensitivity}), $\rho$ increases tenfold, confirming sensitivity to memorization. Across 10 independent MNIST training runs (5\% random forget), $\rho$ has mean $9.72{\times}10^{-4}$ (vs.\ $9.76{\times}10^{-4}$ for the single run of Table~\ref{tab:main}) with standard deviation $2.1{\times}10^{-5}$ (CV $2.2\%$), and MIA has mean $90.4\%$ with s.d.\ $0.3\%$; both are stable but orthogonal, as predicted by Theorem~\ref{thm:unified}.
...
Table~\ref{tab:metric_compare} compares RII and MHPR with existing metrics. RII is the only entry in the table that simultaneously satisfies spectral grounding, hyperparameter-free computation, and $O(CN)$ complexity.
...
In the random-forgetting baseline (Table~\ref{tab:main}), the train/test membership-inference accuracy stays around 90\% while $\rho \approx 10^{-3}$. These are not in tension: the MIA here measures general training membership---the confidence gap between training and test samples---whereas RII measures whether the forget and retain outputs are distinguishable. For a random forget subset the model is unchanged, so the forget and retain outputs remain indistinguishable ($\rho\approx10^{-3}$) even though the train/test membership signal is as strong as in any trained model (90\%). Theorem~\ref{thm:unified} formalizes these as two orthogonal leakage axes.
...
Retrain and NoUnlearn can exhibit near-identical RII while both show high MIA accuracy, because MIA detects training membership generally rather than forget-set-specific erasure. \textbf{RII certifies output-level forget/retain indistinguishability}, the relevant guarantee for compliance.
...
Class-level unlearning verification sits between two failure modes. Accuracy-based metrics saturate and lose their ordering power at high forgetting strength, whereas MIA measures general training membership rather than forget-set--specific output leakage. RII addresses this gap: it is a tuning-free, black-box certificate of output-level indistinguishability that remains informative when accuracy saturates. Empirically, it tracks known forgetting strength across most settings and flags utility-driven collapses, and on TOFU with LLaMA-2-7B it retains that ordering among the checkpoints whose generation accuracy is saturated at 100\%. On MUSE-style long-form book unlearning (Section~\ref{sec:muse}), gradient ascent again leaves the forget output distribution unchanged, forget perplexity ranks retraining as the strongest erasure, and raw-vocabulary channels reflect the two books' inherent distinctness rather than the forgetting state, delimiting when RII is informative. The experiments also delimit what a spectral certificate can and cannot do. RII \emph{rises} under gradient ascent even as accuracy and MIA improve (a failure it exposes, and one that strengthens with class count), it rebounds after ascent-then-fine-tune, and it falls toward zero when a model degenerates, so it should be read together with utility metrics. On the theory side, $\rho=0$ is equivalent to output-channel independence, the fine-tuning bound is $O(\lambda^2)$, and the unified decomposition splits leakage into a direct (MIA) and a cover (RII) axis. The MHPR extension and its temperature scaling handle the known/unknown class asymmetry; a per-sample refinement (PSSD) is developed in Appendix~\ref{app:pssd}.

\textbf{Limitations.} By construction, RII certifies output-level indistinguishability; it does not bound leakage in internal representations, which the probe experiments indicate can persist even for the retraining oracle, and it does not certify parameter-level privacy. The class-level benchmarks are over three seeds (reported as mean $\pm$ std); they support the metric's stability and the qualitative rankings, but with $n{=}3$ they do not by themselves establish statistical significance for close pairs. Seed stability is further corroborated by the random baseline (10 seeds) and the 7B study (3 subsamples). The AG News, 0.5B, and MUSE-style book results are single-run pilot observations reported as transfer evidence rather than full benchmarks (the book study is additionally limited by the base model's prior exposure to the public-domain texts, which compresses the teacher's learnable headroom). The MI bound requires $\pi_{\min}>0$, and the unified decomposition is exact under the Gaussian signal model, holding to leading order more generally.

\textbf{Practical guidance.} RII alone should not be used to declare unlearning successful in an audit. Because the certificate is output-level and can be driven toward zero by model degeneration as well as by erasure, we recommend reporting it alongside: (i) a utility check (retain accuracy, and forget accuracy where available) to distinguish true erasure from collapse; (ii) an MIA score (forget/retain loss AUC, or train/test membership accuracy as appropriate) for the direct leakage axis; (iii) an internal-representation probe (MMD or a residual linear probe on penultimate features) to catch representation-level signatures; and (iv) MHPR when held-out reference classes are available. This two-layer, two-axis audit is what Section~\ref{sec:mia} formalizes.

\textbf{Future work.} Directions include a formal DP--RII connection, extending the author-level TOFU study to the full benchmark suite (forget 90/95/99, Truth Ratio and KLR~\cite{maini2024tofu}) and the MUSE-style book study (Section~\ref{sec:muse}) to the full MUSE~\cite{shi2024muse} suite (copyright books, news), class-level output channels (e.g.\ author- or book-classification heads) for language models, the multi-layer state-trajectory extension of PSSD (Appendix~\ref{app:pssd}), combining RII with internal representation audits for full white-box certification, ImageNet-scale validation, evaluating recent state-of-the-art unlearning methods such as SCRUB~\cite{kurmanji2023scrub} and SalUn~\cite{fan2024salun} alongside the gradient-based baselines, and---when semantically matched held-out classes are unavailable---constructing MHPR reference subspaces from synthetic or generative reference distributions.
...
Since $\delta(x)\in[0,1]$, Hoeffding gives $|\hat{\Delta}_f-\Delta_f|\le\sqrt{\log(2/\zeta)/(2N_f)}$ and $|\hat{\Psi}-\Psi|\le\sqrt{2\log(4/\zeta)/N_{\min}}$ with probability $\ge1-\zeta$. No sub-Gaussian assumption is required, unlike Proposition~\ref{prop:confidence}.
\end{proposition}
...
\label{app:mi_bound}
\begin{proof}
Let $q=P(X{=}f)$, $\bar{q}=1-q$, $P_Y=q\boldsymbol{\mu}_f+\bar{q}\boldsymbol{\mu}_r$, and $\pi_{\min}=\min_y (\boldsymbol{\mu}_r)_y$. In nats, $D_{\mathrm{KL}}\le\chi^2$, so $I(X;Y\mid\theta')\le q\chi^2(\boldsymbol{\mu}_f\|P_Y)+\bar{q}\chi^2(\boldsymbol{\mu}_r\|P_Y)$. Since $P_Y(y)\ge\bar{q}\pi_{\min}$,
\begin{equation}
...
\end{equation}
and symmetrically $\chi^2(\boldsymbol{\mu}_r\|P_Y)\le q\|\boldsymbol{\mu}_f-\boldsymbol{\mu}_r\|_2^2/\pi_{\min}$, so $I\le\frac{2q\bar{q}}{\pi_{\min}}\|\boldsymbol{\mu}_f-\boldsymbol{\mu}_r\|_2^2$. Under the equal-norm condition $\|\boldsymbol{\mu}_f\|_2=\|\boldsymbol{\mu}_r\|_2$, $\|\boldsymbol{\mu}_f-\boldsymbol{\mu}_r\|_2^2=2\sigma_2^2$ (Def.~\ref{def:M}), so $I\le\frac{4q\bar{q}\sigma_2^2}{\pi_{\min}}\le\frac{\sigma_2^2}{\pi_{\min}}=\frac{\sigma_1^2}{\pi_{\min}}\frac{\rho}{1-\rho}$ (since $4q\bar{q}\le1$). As $\rho\to0$ the equal-norm condition holds asymptotically, so $I=O(\rho)$ without it.
\end{proof}
...
Here $\mathbf{J}_{\mathrm{softmax}}$ denotes the Jacobian of the softmax map and $\mathbf{J}_\theta f$ the Jacobian of the logit function with respect to the parameters. With $\mathbf{g}=\nabla_\theta\mathcal{L}(\theta_0,D_f)$ and $\theta'=\theta_0+\lambda\mathbf{g}$, a first-order Taylor expansion gives
\begin{equation}
...
\end{equation}
and $\|\Delta\boldsymbol{\mu}_r\|_2\le\lambda L\|\mathbf{g}\|_2$ for retain samples (uniform softmax Jacobian bound). The squared displacement satisfies $\|\hat{\boldsymbol{\mu}}_f'-\hat{\boldsymbol{\mu}}_r'\|_2^2\le\|\Delta\boldsymbol{\mu}_f\|_2^2+\|\Delta\boldsymbol{\mu}_r\|_2^2+2\|\Delta\boldsymbol{\mu}_f\|_2\|\Delta\boldsymbol{\mu}_r\|_2$ (we omit the first-order cross term with $\boldsymbol{\mu}_f-\boldsymbol{\mu}_r$: in the general case it is at most $O(\lambda^2)$ and therefore does not change the leading $O(\lambda^2)$ bound; for class-level forgetting its expectation is $O(\lambda^3)$ by Lemma~\ref{lem:cross_cov}, so it can be absorbed into the remainder). With $\|\Delta\boldsymbol{\mu}_f\|_2\le\lambda L\|\mathbf{g}\|_2\sqrt{\mathrm{Tr}(\boldsymbol{\Sigma}_f)}$ (under the small-step condition $\lambda\|\mathbf{g}\|_2=O(1)$; equivalently the ascent direction may be normalized and the factor absorbed into $\lambda$),
\begin{equation}
...
\subsection{Proof of Theorem~\ref{thm:unified} (Unified Decomposition)}
\label{app:unified}
\begin{proof}
By the chain rule, $I(X;R) \le I(X;Y) + I(X;R\mid Y)$. For the cover term, the Hilbert--Schmidt decomposition $f_{Y|X}(y|x) = f_Y(y) + \sum_{k\ge 2}\sigma_k\phi_k(y)\psi_k(x)$ gives $\chi^2(P_{Y|X=x}\|P_Y)=\int\frac{(\sum_{k\ge2}\sigma_k\phi_k(y)\psi_k(x))^2}{f_Y(y)}\,dy=\sum_{k\ge2}\sigma_k^2\psi_k(x)^2$; averaging over $X$ and using $\mathbb{E}_X[\psi_k(X)^2]=1$ yields $\mathbb{E}_X[\chi^2(P_{Y|X}\|P_Y)] = \sum_{k\ge 2}\sigma_k^2 = \eta^2$. Since $D_{\mathrm{KL}} \le \chi^2$, we have $I(X;Y) \le \eta^2$. For the direct term, conditioned on $Y=y$, $R = \alpha X + \beta y + N$. By Lemma~\ref{lem:fisher} (Fisher information expansion), $I(X;R\mid Y) = \frac{\alpha^2}{2\sigma^2}\operatorname{Var}(X) + o(\alpha^2)$. Summing yields the decomposition.
\end{proof}

\begin{proof}[Proof of Theorem~\ref{thm:state_bridge}]
From $\mathbf{M}_{\mathrm{sm}} = \mathbf{B}\mathbf{A}$ and the variational characterization $\sigma_i(\mathbf{B}\mathbf{A})=\max_{\dim V=i}\min_{v\in V}\|\mathbf{B}\mathbf{A}v\|_2/\|v\|_2$, the upper bound follows from $\|\mathbf{A}v\|_2\le\tau_1\|v\|_2$: $\sigma_i(\mathbf{B}\mathbf{A})\le\tau_1\sigma_i(\mathbf{B})$. For the lower bound, the right inverse $\mathbf{A}\mathbf{A}^+=\mathbf{I}_m$ gives $\mathbf{B}=\mathbf{B}\mathbf{A}\mathbf{A}^+$, so $\sigma_i(\mathbf{B})\le\|\mathbf{A}^+\|_2\,\sigma_i(\mathbf{B}\mathbf{A})=\tau_m^{-1}\sigma_i(\mathbf{B}\mathbf{A})$, i.e., $\sigma_i(\mathbf{B}\mathbf{A})\ge\tau_m\sigma_i(\mathbf{B})$. Thus $\frac{\tau_m}{\tau_1}\epsilon \le \tilde{\epsilon} \le \frac{\tau_1}{\tau_m}\epsilon$ where $\epsilon=\sigma_2/\sigma_1$, yielding $|\rho_{\mathrm{sm}}-\rho_{\mathcal{S}}| \le 2(\tau_1/\tau_m-1)\rho_{\mathcal{S}} + O((\tau_1/\tau_m-1)^2)$. The $L\delta_q/\|\boldsymbol{\mu}_f\|_2$ term accounts for within-state variance of $\mathbf{a}_s$.
\end{proof}
...
(ii) With $\mathrm{H}^2(P,Q)=1-\sum_i\sqrt{p_iq_i}$, the total variation satisfies $\mathrm{TV}(P_{\delta|f},P_{\delta|r})\le\mathrm{H}(P_{\delta|f},P_{\delta|r})$ (Le Cam). For a threshold classifier, $\mathrm{AUC}=\frac12(1+\mathrm{TPR}-\mathrm{FPR})\ge\frac12(1-\mathrm{TV})$, since over the optimal threshold $\mathrm{TPR}-\mathrm{FPR}\ge1-\mathrm{TV}$. Combining gives the Hellinger bound $\mathrm{AUC}\ge1-\frac12\mathrm{H}^2$.
...
%=============================================================================
\section{Experimental Details and MIA Protocols}
\label{app:details}

\subsection{Visual benchmarks}

Table~\ref{tab:exp_vision} summarizes the visual-benchmark configuration; the class-level benchmarks are run over three seeds (0, 1, 2) and reported as mean $\pm$ std, with no data augmentation and plain per-channel normalization. The normalization constants are: CIFAR-10 mean $(0.4914,0.4822,0.4465)$ and std $(0.2023,0.1994,0.2010)$; Fashion-MNIST and MNIST mean $0.2860$ and std $0.3530$.

\begin{table}[t]
\centering
\small
\caption{Visual-benchmark configuration. ``Class-level'' denotes the protocol of Section~\ref{sec:benchmark}.}
\label{tab:exp_vision}
\setlength{\tabcolsep}{2pt}
\begin{tabular}{l l c c c c}
\toprule
Dataset & Architecture & Optimizer & lr & batch & epochs \\
\midrule
MNIST & SimpleMLP (784$\to$128$\to$10) & Adam & $10^{-3}$ & 64 & 10 \\
Fashion-MNIST & MLP & Adam & $10^{-3}$ & 64 & 10 \\
CIFAR-10 & SmallCNN (3 conv + 2 fc) & Adam & $10^{-3}$ & 64 & 10 \\
CIFAR-100 & SmallCNN / ResNet-18 & Adam & $10^{-3}$ & 64 & 10 \\
MNIST (overfit) & DeepMLP & Adam & $10^{-3}$ & 64 & 50 \\
\bottomrule
\end{tabular}
\end{table}

The random-forgetting baseline (Table~\ref{tab:main}) drops a random 5\% subset; the class-level protocol trains on $\{0,\dots,6\}$ with forget class~3 and held-out classes $\{7,8,9\}$ (CIFAR-10 and Fashion-MNIST), and on a 20-class CIFAR-100 subset with forget class~3 and held-out $\{20,21,22\}$. All experiments run on an Apple Silicon machine (M5 Pro, 24~GB unified memory) with MPS acceleration; the largest visual model (ResNet-18 on CIFAR-100) trains in minutes, so no GPU cluster is required.

\subsection{Language-model experiments}

Table~\ref{tab:exp_llm} summarizes the LLM configuration. The TOFU~\cite{maini2024tofu} author-level protocol uses one forget author (40 QA pairs) and the 90-author retain split; evaluation uses 20 forget and 40 retain QA pairs with a maximum sequence length of 384, repeated over three random evaluation subsamples. LoRA (rank 8, scale 20, dropout 0) is applied to 8 of 32 layers.
...
\label{tab:exp_llm}
\setlength{\tabcolsep}{2pt}
\begin{tabular}{@{}p{1.4cm} p{3.0cm} p{2.8cm} p{4.4cm}@{}}
\toprule
Experiment & Model & PEFT & Unlearning recipe \\
\midrule
TOFU 7B & LLaMA-2-7B (TOFU ckpt) & LoRA $r{=}8$, scale 20, 8/32 layers & NoUnlearn: ckpt as-is; FineTune: AdamW $10^{-5}$, 60 steps, batch 1; NegGrad: SGD $2{\times}10^{-5}$, 10 steps; Retrain: base + LoRA on retain, 60 steps \\
MUSE 7B & LLaMA-2-7B (base) & LoRA $r{=}8$, scale 20, 8/32 layers & Teacher: 200 LoRA steps on forget book; FineTune: AdamW $10^{-5}$, 60 steps; NegGrad: SGD $2{\times}10^{-5}$, 15 steps; Retrain: base + LoRA on retain, 60 steps \\
TOFU 0.5B & Qwen2.5-0.5B & full fine-tune & AdamW $5{\times}10^{-5}$; FineTune 40 steps, NegGrad 8 steps, batch 4 \\
AG News & DistilBERT & full fine-tune & Adam $2{\times}10^{-5}$, 2 epochs, batch 32; 4000 train / 1500 eval per class; forget class = World news \\
\bottomrule
\end{tabular}
\end{table}

The NegGrad strength sweep settings are as described in Section~\ref{sec:llm}.

\subsection{Membership inference attack details}
\label{app:mia}

Two distinct MIA protocols appear in this paper, as stated by each table's caption and by the ``Notation for MIA'' paragraph in the Setup of Section~\ref{sec:experiments}.

\textbf{Loss-threshold AUC (class-level benchmark, Table~\ref{tab:benchmark}).} For each forget and retain sample we compute the model's cross-entropy loss; the score of every sample is its loss, and the reported MIA-loss is the AUC obtained by thresholding these scores (forget samples are the positive class). No shadow model or retraining is involved: the target model is evaluated directly. Because the forget class is an unseen class in the training protocol, its losses exceed those of retain samples, giving the ``reverse'' ordering discussed in Section~\ref{sec:benchmark} (the oracle attains AUC${\to}0$, which is itself evidence of successful erasure).

\textbf{Train/test membership accuracy (random baseline, Table~\ref{tab:main}).} We score each sample by its softmax confidence (or, equivalently, negative loss) and classify training vs.\ test membership by thresholding; the reported value is the average membership accuracy over the two classes. This measures general memorization (the train/test confidence gap), which is orthogonal to forget/retain indistinguishability (Section~\ref{sec:mia}).

\textbf{Why a simple attack suffices.} The goal of this paper is to rank unlearning methods, not to maximize attack success, and the certificate RII is a distributional quantity rather than an adversarial bound. Stronger attacks such as LiRA or the first-principles analyses of Carlini et al.~\cite{carlini2022membership} require shadow models or repeated training runs; under a unified single-model protocol the added power does not change the relative ranking of methods, because the output-level signal available to any black-box attacker is the same per-sample loss or confidence that the simple threshold attacks use. We therefore report the simple attacks and leave stronger-attack comparisons to future work.

%=============================================================================
% REFERENCES (BibTeX, spmpsci style)
%=============================================================================
\bibliographystyle{spmpsci}      % Springer math & physical sciences style
