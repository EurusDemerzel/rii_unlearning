...
\noindent
\textbf{摘要（逐句对照）：}
机器学习遗忘旨在无需完全重训练的情况下，从已训练模型中移除特定训练数据的影响；验证这种移除是否成功是一个核心开放问题。成员推理攻击（MIA）将训练/测试分布位移与遗忘集泄漏混为一谈，而差分隐私界要求侵入式训练，因此二者都无法证明输出级不可区分性。我们引入一个基于 $2\times C$ \emph{经验输出通道矩阵}的谱证书，该矩阵的两行分别是遗忘集与保留集上的平均 softmax 预测，并定义 \textbf{残差不可逆指数（Residual Irreversibility Index, RII）} $\rho=1-\sigma_1^2/(\sigma_1^2+\sigma_2^2)$，当两个通道输出分布不可区分时它精确地消失。对于类级遗忘（标准 RII 在此遭受已知/未知类不对称），我们引入 \textbf{多留出投影残差（Multi-Held-Out Projection Residual, MHPR）} $\rho_H$，将遗忘类均值投影到多个留出未见类的子空间上，并给出有限样本保证与面向低精度情形的温度缩放。我们证明 $\rho=0$ 等价于输出通道独立，并导出微调泄漏界、互信息界，以及输出泄漏的统一两轴分解。在跨 CIFAR-10、Fashion-MNIST 与 CIFAR-100（20 类）的统一基准上，以及在标准 TOFU 基准~\cite{maini2024tofu} 与 MUSE 风格长文本书籍遗忘协议上的 70 亿参数语言模型（LLaMA-2-7B，基于 LoRA 的 PEFT）上，RII 在大多数设置中对遗忘方法的排序与已知遗忘强度一致，并在准确率与 MIA 饱和处仍具信息量。当已遗忘检查点将遗忘集生成准确率饱和在 100\% 时，准确率本身无法对它们排序，而 RII 仍将微调排序在未遗忘之下。RII 还揭示出梯度上升的一个隐藏失败：其输出签名在准确率与 MIA 都报告改善的同时反而增强，并伴随一个基于准确率的指标无法发现的微调反弹。在长文本书籍遗忘（MUSE 风格）中，梯度上升使遗忘输出分布保持不变，遗忘困惑度将重训练排序为最强擦除，而原始词表通道携带的是两本书的固有差异而非遗忘状态——这是证书的一个清晰边界，印证通道语义决定其判别力。表示级探针证实输出级擦除可以与特征级签名共存，这促使采用两层审计。
...
我们提出一个基于\textbf{谱条件}的轻量黑盒证书：输出空间不可逆性归结为 $2\times C$ 输出通道矩阵的秩。所谓\textbf{输出分布不可逆性}，我们指遗忘集成员指示变量 $X$ 与模型在参数 $\theta'$ 下的类别输出 $Y$ 条件独立，即 $I(X;Y\mid\theta')=0$。当此成立时，模型在 $D_f$ 与 $D_r$ 上的输出分布在统计上不可区分，产生一个秩一输出通道矩阵且 $\rho=0$。
...
框架建立在一个简单观察之上。$2\times C$ \textbf{经验输出通道矩阵} $\mathbf{M}(\theta')$ 的行是 $\boldsymbol{\mu}_f$ 与 $\boldsymbol{\mu}_r$（$D_f$ 与 $D_r$ 上的平均 softmax 向量）。其奇异值 $\sigma_1 \ge \sigma_2$ 编码残差依赖：$\sigma_2 = 0 \iff \mathrm{rank}(\mathbf{M}) = 1 \iff$ 完美移除。

\textbf{贡献：}
\begin{enumerate}
\item 一个输出谱框架（第~\ref{sec:channel}~节）：一个 $2\times C$ 通道矩阵，其秩一条件给出无需调参的残差不可逆指数（RII）。
\item 一个证明（定理~\ref{thm:perfect}）：$\rho=0$ 当且仅当 $I(X;Y\mid\theta')=0$，论证是通道级的，无需指数族或校准假设。
...
\item 对 RII 与 MHPR 在统一七方法基准以及 TOFU 与 MUSE 风格书籍遗忘基准的 70 亿参数语言模型上的评估（第~\ref{sec:experiments}~节），揭示准确率与 MIA 都会错过的隐藏失败（梯度上升签名增强与微调反弹）；在 TOFU 上，RII 对生成准确率饱和于 100\% 的已遗忘检查点进行排序，而准确率本身不提供任何排序，且在长文本书籍遗忘中梯度上升同样使遗忘输出分布保持不变。我们还给出 $O(CN)$ 审计协议与逐样本扩展，逐样本状态差异（Per-Sample State Disparity, PSSD；附录~\ref{app:pssd}）。
\end{enumerate}
...
设 $D = \{(x_i, y_i)\}_{i=1}^N$ 为训练集，$\theta_0 = \mathcal{A}(D)$ 为训练所得参数，$D_f \subset D$ 为遗忘集，$D_r = D \setminus D_f$ 为保留集，并令 $N_f = |D_f|$、$N_r = |D_r|$。遗忘算法 $\mathcal{U}$ 产生 $\theta' = \mathcal{U}(\theta_0, D_f, D_r)$。我们通过输出分布来量化残差依赖 $I(D_f; \theta' \mid \theta_0)$。
...
定义~\ref{def:rii}（式~\eqref{eq:rho}）将 $\rho$ 定义为一种谱残差能量（输出通道的归一化第二奇异值），而非互信息值。其信息论含义由定理~\ref{thm:mi_bound} 确立：当 $\rho\ll1$ 时 $I(X;Y\mid\theta')\le O(\rho)$，因此 $\rho$ 在互信息消失时精确消失，并在小泄漏时上界它。
...
在类级遗忘中，遗忘行与保留行平均的是根本不同的分布：$\boldsymbol{\mu}_f$ 来自模型训练中从未见过的类。因此即使完美遗忘，两行也可能不同，从而抬高标准 RII。
...
表~\ref{tab:assumptions} 列出每个理论结果背后的假设及其在本文实验设定中的状态；随后我们讨论审稿人可能质疑的假设。
...
\label{rem:pi_min}
对经验通道，$(\boldsymbol{\mu}_r)_y$ 是类 $y$ 在保留集上的相对频率，因此只要每个类至少出现一次（本文所有实验的情形）就有 $\pi_{\min}>0$。对类数非常多、某些类概率任意小的人群通道，可将该界应用于截断支撑 $\mathcal{Y}_\epsilon=\{y:(\boldsymbol{\mu}_r)_y>\epsilon\}$，代价是 $O(1/\epsilon)$ 的乘法因子；同样的截断出现在置信界中（命题~\ref{prop:confidence}）。
\end{remark}
