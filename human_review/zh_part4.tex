...
MUSE 7B & LLaMA-2-7B（基座） & LoRA $r{=}8$，scale 20，8/32 层 & Teacher：遗忘书籍上 200 步 LoRA；NoUnlearn：teacher 原样；FineTune：AdamW $10^{-5}$，60 步；NegGrad：SGD $2{\times}10^{-5}$，15 步；Retrain：基座 + LoRA on retain，60 步 \\
...
NegGrad 强度扫描（第~\ref{sec:llm}~节）将步数 $\times$ 学习率从基线 $(10, 2{\times}10^{-5})$ 变化到 $(30, 10^{-4})$（$3\times$ 步数，$5\times$ lr）与 $(60, 2{\times}10^{-4})$（$6\times$ 步数，$10\times$ lr），始终从全新的零 LoRA 出发。
