...
缩写``MIA''在本文中表示三种不同的协议，每个表格都标明其所指：随机遗忘基线中的训练/测试成员推断准确率（表~\ref{tab:main}）；类级基准中的遗忘/保留损失阈值 AUC（表~\ref{tab:benchmark}）；以及 TOFU 上的遗忘集平均下一词 NLL（表~\ref{tab:llm}）。在对比两条泄漏轴时（第~\ref{sec:mia}~节），``MIA''指训练/测试成员准确率。
...
表~\ref{tab:main} 揭示四点关键发现。第一，$\rho<5\times10^{-3}$ 一致成立：标准神经网络训练产生近秩一输出通道。第二，$\rho$ 与准确率无关：CIFAR-100 在 20\% 准确率下给出的 $\rho$ \emph{低于} MNIST 在 97.5\% 下的值，证实秩一性质是 softmax 输出单纯形固有的。第三，不同遗忘方法给出几乎相同的 $\rho$（我们在 MNIST 上验证了 Retrain、SISA 与 FineTune），这是预期的：对随机子集，模型的泛化使输出即使不做显式遗忘也不可区分。第四，在 10 个随机种子下，$\rho$ 的标准差 $<3.5\times10^{-4}$，支持可靠的单次审计。因此在随机遗忘下 RII 近乎平凡（所有方法不可区分），而类级遗忘正是 RII 与 MHPR 变得可判别的区间（第~\ref{sec:mhpr}~节与第~\ref{sec:benchmark}~节）。

\subsection{70 亿参数语言模型上的 RII（TOFU）}
\label{sec:llm}

上述基准是视觉的；我们接下来检验该证书能否经受尺度与模态的迁移，即迁移到当代指令微调语言模型。我们在 \emph{LLaMA-2-7B}（官方 TOFU 微调检查点）上运行标准 TOFU~\cite{maini2024tofu} 作者级协议，遗忘集 $=$ 一位作者（40 个 QA 对），保留划分为 90 位作者，采用实践者实际部署的 PEFT 方案：LoRA（$r{=}8$，scale~20，作用于 32 层中的 8 层）。方法：NoUnlearn（检查点原样）、FineTune（在保留答案上 60 步 LoRA，AdamW $10^{-5}$）、NegGrad（在遗忘答案上 10 步 LoRA 梯度上升，SGD $2{\times}10^{-5}$），以及 Retrain oracle（基座 LLaMA-2-7B 加仅在保留集上的 LoRA；基座模型从未见过遗忘作者）。RII 在遗忘与保留 QA 对的平均下一 token softmax 分布构成的 $2\times V$（$V{=}32{,}000$）通道矩阵上计算；MIA 为遗忘集的平均下一 token NLL。
...
三个遗忘后的检查点（以及未触动模型）都能完美回答遗忘问题（Forget$=$100\%；许多 TOFU 答案可以直接从问题本身恢复），因此生成准确率无法区分它们。相比之下，RII 将 FineTune 排在 NoUnlearn 之下（$9.73{\times}10^{-2}$ 对比 $1.16{\times}10^{-1}$，$-16\%$），与视觉基准的排序相同，并将（轻量、60 步的）重训练 oracle（$8.01{\times}10^{-2}$）与未触动模型区分开 $1.45\times$。oracle 另外还可由生成准确率（80\% 对比 100\%）与其大得多的 MIA（2.163 对比 1.348）区分；此处的声明更窄：在生成准确率饱和于 100\% 的检查点中，RII 提供了准确率无法给出的排序。我们不声称仅凭 RII 就能区分 NegGrad 与 NoUnlearn：它们的差距（$1.7{\times}10^{-3}$）落在评估噪声内（见下文）。

NegGrad 的 RII（$1.17{\times}10^{-1}$）在评估噪声内与 NoUnlearn（$1.16{\times}10^{-1}$）不可区分，不同于 FineTune 的 16\% 下降；其 MIA（1.352）同样匹配 NoUnlearn（1.348）。因此上升在此不产生可测量的输出空间擦除，而下面的强度扫描表明，随着上升加强，遗忘输出签名\emph{增强}（在 $3\times$ 强度时升至 $1.20{\times}10^{-1}$，随后模型崩溃），这是第~\ref{sec:benchmark}~节 CIFAR-10/100 上所述隐藏失败的 7B 类比。

在保留答案上继续 LoRA 训练使遗忘与保留输出分布靠拢（$\rho{=}9.73{\times}10^{-2}$，$-16\%$）并取得最低 MIA（0.852；模型对遗忘集仍最流畅），与视觉结果一致；如第~\ref{sec:rebound}~节所述，RII 降低是真擦除的必要而非充分条件，因为遗忘准确率仍为 100\%。
...
我们还对梯度上升强度（步数 $\times$ 学习率）做扫描，从基线 $10\times 2{\times}10^{-5}$ 到 $30\times 10^{-4}$（$3\times$ 步数，$5\times$ lr）与 $60\times 2{\times}10^{-4}$（$6\times$ 步数，$10\times$ lr），始终从全新的零 LoRA 出发。在中等强度下，RII 进一步\emph{上升}到 $1.20{\times}10^{-1}$（高于 NoUnlearn 的 $1.16{\times}10^{-1}$），而遗忘/保留准确率保持在 100\%：隐藏失败并不饱和——更强的上升锐化遗忘输出签名。在最强设置下模型开始崩溃：MIA 跳到 5.25（从 1.35），上升损失发散到 6.5，而 RII 回落到 $1.01{\times}10^{-1}$，但生成准确率仍是 100\%，因此仅凭准确率仍无法检测崩溃。此处 RII 下降\emph{是因为输出分布退化为近随机噪声}，这巧合地使两个集合看起来又统计相似；崩溃反而由 MIA 标记。因此 RII 是双向的：它随隐藏失败签名增强而上升，随输出分布本身退化而下降，所以低 RII 必须与效用指标（保留准确率、MIA）一起解读，以区分真擦除与模型崩溃，这与第~\ref{sec:mia}~节的两轴审计一致。

\subsection{MUSE 风格长文本书籍遗忘上的 RII}
\label{sec:muse}

TOFU 是基于问答的（共享格式的短提示）。为检验证书在 MUSE~\cite{shi2024muse} 精神下的长文本知识遗忘（模型先记住一本书，然后被要求遗忘它），我们使用公有领域书籍（Project Gutenberg）。遗忘集为《爱丽丝梦游仙境》（Carroll, 1865）；保留集为《镜中世界》（Carroll, 1871），它是同一作者的续作，风格与遗忘文本可比（使用不同作者《绿野仙踪》的变体给出相同的定性结论，见下文）。teacher 为基座 LLaMA-2-7B 加遗忘书籍上的 200 步 LoRA；四种检查点其余与第~\ref{sec:llm}~节相同：NoUnlearn（teacher 原样）、FineTune（保留书籍上 60 步 LoRA）、NegGrad（遗忘书籍上 15 步 LoRA 梯度上升）、Retrain（基座加仅在保留书籍上的 LoRA）。所有文本用 LLaMA-2 tokenizer 分块；留出块（每集 25 个）与训练块不相交。RII 照旧在 $2\times V$ 通道矩阵上计算；遗忘/保留困惑度为平均下一 token NLL 的 $\exp$，MIA 为遗忘集平均 NLL。

\begin{table}[t]
\centering
\small
\caption{LLaMA-2-7B 上 MUSE 风格长文本书籍遗忘（遗忘：《爱丽丝梦游仙境》；保留：《镜中世界》；基于 LoRA；Retrain 为轻量 60 步 oracle）。}\label{tab:muse}
\setlength{\tabcolsep}{2pt}
\begin{tabular}{l c c c c}
\toprule
方法 & 遗忘PPL & 保留PPL & RII & MIA \\
\midrule
...
\bottomrule
\end{tabular}
\par\vspace{2pt}\footnotesize 遗忘PPL/保留PPL 为留出块上平均下一 token NLL 的 $\exp$；遗忘困惑度越高表示遗忘越强。MIA 为遗忘集平均下一 token NLL（越高表示遗忘越强）。粗体标记每列最被遗忘的检查点。
\end{table}

表~\ref{tab:muse} 报告了结果。teacher 确实学会了遗忘书籍：遗忘困惑度从 2.11（基座）降到 1.97，而保留困惑度保持在基座水平附近（3.52 到 3.39）。余量很小，因为这些公有领域书籍已存在于基座模型的预训练语料中——这是使用真实书籍的一个诚实局限。在遗忘下，遗忘困惑度将 Retrain 排序为最强擦除（2.00，最高值），其次为 FineTune（1.96）与 NoUnlearn（1.97）；MIA 一致（Retrain 0.691，其余 0.674--0.676）。NegGrad 是显著结果：15 步上升后——在单独运行中 50 步后——其遗忘困惑度、MIA 与 RII 与 NoUnlearn 在四位有效数字内不可区分（1.97、0.676、$1.18{\times}10^{-2}$）。因此梯度上升在长文本上同样不产生可测量的输出空间擦除，这是第~\ref{sec:llm}~节 TOFU 结果的书籍域类比。FineTune 反而降低保留困惑度（3.39 到 3.22，$-5\%$）同时使遗忘困惑度基本不变（1.97 到 1.96），再次匹配 TOFU 中持续微调保留遗忘流畅度的模式。

原始词表通道上的 RII \emph{无法}分离这些检查点。所有方法落在 $[1.05{\times}10^{-2}, 1.26{\times}10^{-2}]$，未触动基座模型的参考值（$1.34{\times}10^{-2}$）高于其中大多数；按困惑度是最强擦除的 Retrain 反而具有\emph{最高} RII（$1.26{\times}10^{-2}$）。原因是结构性的：遗忘与保留是两本不同的书，因此无论模型记住什么，它们的下一 token 分布都不同——RII 度量的是书籍的固有差异而非遗忘状态。同作者保留将基线减半（从不同作者《绿野仙踪》保留的 $2.58{\times}10^{-2}$ 到《镜中世界》的 $1.34{\times}10^{-2}$），但原始词表通道仍无法对方法排序。这是注记~\ref{rem:channel} 预测的长文本情形：通道语义决定判别力，原始词表通道仅在遗忘与保留集诱导可比输出分布时（如 TOFU 的 QA 对）才具信息量；对不同的书籍，类级输出头（例如书籍分类头）是推荐的审计对象。因此我们报告书籍研究作为 RII 适用处与必须补充处的证据，连同确实成立的困惑度/MIA 排序；在两种保留变体（同作者与不同作者）上，NegGrad 都与 NoUnlearn 保持不可区分，Retrain 取得最高遗忘困惑度。
...
表~\ref{tab:cifar100} 用 20 个训练类重复该协议（遗忘类~3，留出 $\{20,21,22\}$）。NegGrad 隐藏失败在更大规模下\emph{更显著}：其 RII（$0.189{\pm}0.016$）甚至超过 NoUnlearn（$0.127{\pm}0.026$，$1.49\times$ 差距，对比 CIFAR-10 的 $1.31\times$），而遗忘准确率与 MIA-loss 再次改善：被梯度上升增强的输出签名在更大的输出空间中更尖锐。已知/未知不对称持续（oracle $\rho{=}0.131{\pm}0.002$）。然而，MHPR 在此规模不再分离 oracle（$0.187{\pm}0.157$）与 NoUnlearn（$0.152{\pm}0.096$）——其方差随类数增大——因此谱 RII 分离在此是更稳健的信号。报告六种方法；SISA 因 20 类的分片训练在此规模不成比例而省略。
...
上述基准是视觉的。为检验跨模态迁移，我们在文本上重复该协议。在 AG News（四类，遗忘集 $=$ World 新闻）上，微调两轮 epoch 的 DistilBERT 分类器~\cite{sanh2020distilbert} 显示与视觉相同的排序：RII 将未触动模型（$\rho{=}0.267$）与重训练 oracle（$\rho{=}0.090$）区分开，$2.96{\times}$ 差距与 CIFAR-10 结果一致。这些文本结果是单次运行的 pilot 观察，作为迁移证据而非完整的跨模态基准报告，因为我们未在多个种子上重复。梯度上升在此行为不同：它彻底摧毁模型（保留准确率降到 $37.5\%$，$\rho{\approx}0$），是显性而非隐藏失败。因此低 RII 是擦除的必要而非充分证据：必须与保留准确率一起解读，才能区分真移除与模型崩溃，这与第~\ref{sec:mia}~节的两轴观点一致。同样的情况出现在小语言模型（Qwen2.5-0.5B~\cite{qwen2024qwen2}）的作者级 TOFU~\cite{maini2024tofu} 协议下：暴露后在保留集上微调使遗忘答案保持不变（准确率不变），在仅解码器模型中复现第~\ref{sec:rebound}~节的反弹，而梯度上升再次摧毁模型（$\rho{\to}0$，损失增长一个数量级）。这个小模型 pilot 也暴露出一个真实的边界：在 0.5B 规模下，词表级输出通道无法将 oracle 与未触动模型区分开（$\rho{=}0.080$ 对比 $0.083$），因为两个答案集诱导出统计上相似的文本分布。第~\ref{sec:llm}~节表明在 7B 规模下相同词表通道变得可判别：证书的判别力在于输出通道的\emph{类语义}，而模型容量决定原始词表通道是否足够（参见注记~\ref{rem:channel}）；语言模型审计应优先使用类级通道（例如作者分类头）。
...
表~\ref{tab:mhpr} 比较 MHPR 与 LOCO。MHPR 将遗忘类均值投影到 $K$ 个留出未见类的张成子空间上（第~\ref{sec:mhpr}~节）；在 MNIST 类遗忘上，$K\ge3$ 解决 LOCO 失败：$K{=}1$ 给出 $\rho_H{=}0.942$（比 LOCO 更差，因为单个参照错失未见类流形），而 $K{=}3$ 将 $\rho_H$ 降到 $0.046$，比 $\rho_{\mathrm{std}}{=}0.136$ 与 $\rho_{\mathrm{LOCO}}{=}0.310$ 低一个数量级。$K{=}4$ 达到 $0.0008$（近机器零）但消耗训练数据（准确率 $79.5\%{\to}48.7\%$）。我们推荐 $K{=}3$。图~\ref{fig:k_ablation} 以图形方式展示相同趋势。

\begin{figure}[t]
\centering
\includegraphics[width=0.78\textwidth]{../ai_submission/figures/k_ablation.pdf}
\caption{MHPR 随留出类数量 $K$ 的变化（MNIST 类遗忘，遗忘类~5，对数刻度）。$\rho_H$ 在 $K{=}3$ 时下降两个数量级，$K{=}4$ 时接近机器零，而标准与 LOCO RII 基线保持在 $0.14$ 与 $0.31$ 附近（表~\ref{tab:mhpr}）。}\label{fig:k_ablation}
\end{figure}
...
\paragraph{跨数据集与敏感性。}对 $C{=}10$--$100$（MNIST、CIFAR-10/100），$\rho\le2\times10^{-3}$，证实秩一性质是结构性的。在刻意过拟合下（表~\ref{tab:sensitivity}，第~\ref{app:extra}~节），$\rho$ 飙升 $10\times$（MNIST 60K$\to$5K，50 epoch：$9.8\times10^{-4}\to1.24\times10^{-2}$）。受控 $\alpha$-扰动证实正交轴：$\alpha{=}0.20$ 使 RII 提高 $9\times$ 而 MIA 仅上升 $6.8$ 个百分点。CIFAR-100 上的 ResNet-18 保持 $\rho{=}1.41\times10^{-3}$（表~\ref{tab:resnet18}），证实架构独立性。
...
在刻意过拟合下（表~\ref{tab:sensitivity}），$\rho$ 飙升 $10\times$，证实对记忆化的敏感性。在 10 次独立 MNIST 训练运行（5\% 随机遗忘）中，$\rho$ 均值 $9.72{\times}10^{-4}$、标准差 $2.1{\times}10^{-5}$（CV $2.2\%$），MIA 均值 $90.4\%$、标准差 $0.3\%$；两者都稳定但正交，如定理~\ref{thm:unified} 所预测。
