...
\begin{proof}[Proof Sketch]
The estimate $\hat{\rho}_H$ depends on $K+1$ mean vectors ($\boldsymbol{\mu}_f$ and $\boldsymbol{\mu}_{h_1},\dots,\boldsymbol{\mu}_{h_K}$), each estimated from $N_f$ or $N_{h_k}$ sub-Gaussian samples. By Hoeffding's inequality and a union bound over $(K+1)C$ entries, $\max(\|\hat{\boldsymbol{\mu}}_f - \boldsymbol{\mu}_f^*\|_2, \max_k \|\hat{\boldsymbol{\mu}}_{h_k} - \boldsymbol{\mu}_{h_k}^*\|_2) \le 4\nu\sqrt{C\log((K+1)C/\delta)/N_{\min}}$ with probability $\ge 1-\delta$. The MHPR is a locally Lipschitz function of these $K+1$ vectors. A direct computation gives $\|\nabla_{\boldsymbol{\mu}_f}\phi\|_2 \le 2/\|\boldsymbol{\mu}_f\|_2$ and $\|\nabla_{\boldsymbol{\mu}_{h_k}}\phi\|_2 \le 2/\|\boldsymbol{\mu}_f\|_2$. With $\|\boldsymbol{\mu}_f\|_2 \ge \|\boldsymbol{\mu}_f^*\|_2/2$ for sufficiently large $N_f$, the combined Lipschitz constant is $L = 8/\|\boldsymbol{\mu}_f^*\|_2^2$. Applying the delta method yields the claimed bound. See Appendix~\ref{app:mhpr_proofs} for details.
\end{proof}
...
\textbf{Connection to Theorem~\ref{thm:unified}.} When applying Lemma~\ref{lem:fisher} in the proof of Theorem~\ref{thm:unified}, we condition on $Y$ so that $Z = N$ (the only remaining randomness is the observation noise). Thus $J(Z) = J(N) = 1/\sigma^2$.
...
\begin{proof}
$\rho = 0 \Rightarrow \sigma_2 = 0 \Rightarrow \mathrm{rank}(\mathbf{M}) = 1$. Since both rows are probability vectors summing to one, a rank-one $2\times C$ matrix must have proportional rows: $\boldsymbol{\mu}_f = \lambda \boldsymbol{\mu}_r$ for some $\lambda > 0$. The normalization $\sum_c (\boldsymbol{\mu}_f)_c = \sum_c (\boldsymbol{\mu}_r)_c = 1$ forces $\lambda = 1$, hence $\boldsymbol{\mu}_f = \boldsymbol{\mu}_r$.

For the softmax output (a categorical exponential family), the mean parameter $\boldsymbol{\mu} = \mathbb{E}[Y\mid x] = \mathbf{p}(x;\theta')$ uniquely determines the conditional distribution via the bijective softmax link. Hence $\boldsymbol{\mu}_f = \boldsymbol{\mu}_r$ implies equality of the full conditional distributions: $P(Y\mid x\in D_f, \theta') = P(Y\mid x\in D_r, \theta')$. Therefore $Y \perp X \mid \theta'$, i.e., $I(X; Y \mid \theta') = 0$. The converse follows because $\boldsymbol{\mu}_f \neq \boldsymbol{\mu}_r$ creates a binary channel with positive capacity.
\end{proof}
...
\begin{proof}[Proof Sketch]
Each entry of $\hat{\mathbf{M}}$ is a sample mean of $N_f$ or $N_r$ sub-Gaussian variables. By Hoeffding's inequality, $\mathbb{P}(|\hat{M}_{ij} - M_{ij}| \ge t) \le 2\exp(-N_{\min} t^2 / 2\nu^2)$ for each entry. A union bound over $2C$ entries gives $\|\hat{\mathbf{M}} - \mathbf{M}\|_F \le 2\nu\sqrt{2C\log(4C/\delta)/N_{\min}}$ with probability $\ge 1-\delta$. By Weyl's perturbation theorem, $|\hat{\sigma}_i - \sigma_i| \le \|\hat{\mathbf{M}} - \mathbf{M}\|_2 \le \|\hat{\mathbf{M}} - \mathbf{M}\|_F$. The RII $\rho = f(\sigma_1,\sigma_2) = 1 - \sigma_1^2/(\sigma_1^2+\sigma_2^2)$ has gradient $\|\nabla f\|_2 \approx 2\sigma_2/\sigma_1^2$ for $\rho \ll 1$, giving a Lipschitz constant $O(1/\sigma_1^2)$. The delta method then yields the claimed rate. See Appendix~\ref{sec:appendix} for the explicit constant derivation.
\end{proof}
...
\label{eq:mi_bound}
\end{equation}
where $\delta = \min_y P_r(y) > 0$. For well-calibrated classifiers ($\delta \approx 1/C$): $I \le \frac{2C\sigma_1^2}{\ln 2}\cdot\frac{\rho}{1-\rho}$. For $\rho \ll 1$: $I \le O(\rho)$. The $o(\rho)$ term vanishes as $\rho \to 0$; in particular, $\rho=0$ implies $I=0$, consistent with Theorem~\ref{thm:perfect}.
\end{theorem}
...
\begin{proof}
By the chain rule and data processing inequality, $I(X;R) \le I(X;Y) + I(X;R\mid Y)$. For the cover term, the Hilbert--Schmidt decomposition yields $I(X;Y) \le C_2\sum_{k\ge 2}\sigma_k^2 = C_2\eta^2$, where $C_2$ is a universal constant depending only on the output space (for discrete $Y$ with $C$ classes, $C_2 = \log C$); this follows from bounding mutual information by the squared Hilbert--Schmidt norm of the deviation from rank one~\cite{bloch2021overview}. For the direct term, Lemma~\ref{lem:fisher} gives $I(X;R\mid Y) = \frac{\alpha^2}{2\sigma^2}\mathrm{Var}(X) + o(\alpha^2)$, noting that conditioned on $Y$, the only remaining randomness is the Gaussian noise $N$, so $J(Z)=J(N)=1/\sigma^2$. Summing both contributions yields the decomposition.
\end{proof}
